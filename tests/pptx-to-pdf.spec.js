...
test("PowerPoint to PDF tool loads and has correct UI", async ({ page }) => {
  await page.goto("http://localhost:3000/#/tools/pptx-to-pdf");
  ...
  expect(title).toContain("PowerPoint to PDF");

  // Check description
  const desc = await page.locator(".tool-description").textContent();
  expect(desc).toContain("Convert .pptx presentations to PDF files");
  ...
  console.log("✅ PowerPoint to PDF tool loads correctly");
});
