...
test('PNG to JPG Converter tool loads and has correct UI', async ({ page }) => {
  await page.goto('http://localhost:3000/#/tools/png-to-jpg');
  ...
  expect(title).toContain('PNG to JPG');
  
  // Check description
  const desc = await page.locator('.tool-description').textContent();
  expect(desc).toContain('Convert PNG images to JPG with quality control');
  ...
  console.log('✅ PNG to JPG Converter tool loads correctly');
});
