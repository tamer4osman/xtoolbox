...
test('JPG to WebP Converter tool loads and has correct UI', async ({ page }) => {
  await page.goto('http://localhost:3000/#/tools/jpg-to-webp');
  ...
  expect(title).toContain('JPG to WebP');
  
  // Check description
  const desc = await page.locator('.tool-description').textContent();
  expect(desc).toContain('Convert JPG images to WebP with quality control');
