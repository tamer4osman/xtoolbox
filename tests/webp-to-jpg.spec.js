...
test('WebP to JPG Converter tool loads and has correct UI', async ({ page }) => {
  await page.goto('http://localhost:3000/#/tools/webp-to-jpg');
  ...
  expect(title).toContain('WebP to JPG');
  
  // Check description
  const desc = await page.locator('.tool-description').textContent();
  expect(desc).toContain('Convert WebP images to JPG with quality control');
