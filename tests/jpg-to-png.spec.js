...
test('JPG to PNG Converter tool loads and has correct UI', async ({ page }) => {
  await page.goto('http://localhost:3000/#/tools/jpg-to-png');
  ...
  expect(title).toContain('JPG to PNG');
  
  // Check description
  const desc = await page.locator('.tool-description').textContent();
  expect(desc).toContain('Convert JPG images to lossless PNG format');
  ...
  console.log('✅ JPG to PNG Converter tool loads correctly');
});
