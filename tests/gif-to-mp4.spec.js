...
test('GIF to MP4 tool loads and has correct UI', async ({ page }) => {
  await page.goto('http://localhost:3000/#/tools/gif-to-mp4');

  await page.waitForSelector('.tool-layout', { timeout: 5000 });

  const title = await page.locator('.tool-header h1').textContent();
  expect(title).toContain('GIF to MP4');

  const desc = await page.locator('.tool-description').textContent();
  expect(desc).toContain('Convert animated GIF to MP4');
  ...
  console.log('✅ GIF to MP4 tool loads correctly');
});
