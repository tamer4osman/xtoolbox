...
test('PNG to PDF Converter tool loads and has correct UI', async ({ page }) => {
  await page.goto('http://localhost:3000/#/tools/png-to-pdf');
  ...
  expect(title).toContain('PNG to PDF');
  
  // Check description
  const desc = await page.locator('.tool-description').textContent();
  expect(desc).toContain('Convert PNG images to PDF documents');
  ...
  console.log('✅ PNG to PDF Converter tool loads correctly');
});
