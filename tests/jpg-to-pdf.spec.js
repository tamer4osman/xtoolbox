...
test("JPG to PDF Converter tool loads and has correct UI", async ({ page }) => {
  ...
  expect(title).toContain("JPG to PDF");

  // Check description
  const desc = await page.locator(".tool-description").textContent();
  expect(desc).toContain("Convert JPG images to PDF documents");
  ...
  console.log("✅ JPG to PDF Converter tool loads correctly");
});
