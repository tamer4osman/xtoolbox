...
describe('barcode-scanner scan flow', () => {
  it('shows decoded text and format name on successful scan', async () => {
    ...
    expect(container.querySelector('#barcode-scanner-result-content').textContent).toBe('012345678905');
    expect(container.querySelector('#barcode-scanner-result-format').textContent).toBe('Format: EAN_13');
    expect(container.querySelector('#barcode-scanner-result').classList.contains('hidden')).toBe(false);
  });
  ...
  it('shows error when scan fails', async () => {
    ...
    expect(container.querySelector('#barcode-scanner-error-message').textContent).toMatch(/no barcode/i);
  });
});
