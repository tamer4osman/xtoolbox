...
describe('codec-factory', () => {
  ...
  it('returns an init function', () => {
    const init = createCodecTool({
      ...
      encodeId: 'encode-btn', decodeId: 'decode-btn',
      copyId: 'copy-btn', clearId: 'clear-btn',
      encode: s => s.toUpperCase(), decode: s => s.toLowerCase()
    });
  ...
  it('shows error when encode throws', () => {
    const init = createCodecTool({
      ...
      encodeId: 'encode-btn', decodeId: 'decode-btn',
      copyId: 'copy-btn', clearId: 'clear-btn',
      encode: () => { throw new Error('bad input'); }, decode: s => s
    });
    init();
  ...
  it('returns early when elements are missing', () => {
    const init = createCodecTool({
      inputId: 'nonexistent', outputId: 'nonexistent',
      encodeId: null, decodeId: null,
      copyId: null, clearId: null,
      encode: s => s, decode: s => s
    });
