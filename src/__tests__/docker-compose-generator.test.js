...
describe('docker-compose-generator: TEMPLATES', () => {
  ...
  it('database templates include a password env var', () => {
    ...
    for (const k of dbs) {
      ...
      expect(env.some(v => /PASSWORD/i.test(v)), `${k} should include a password env var`).toBe(true);
    }
  });
});
...
describe('docker-compose-generator: buildCompose', () => {
  ...
  it('emits a top-level networks block with project name suffix', () => {
    const out = buildCompose({
      projectName: 'myapp',
      services: [
        { name: 'web', image: 'nginx:alpine', command: '', ports: [], volumes: [], environment: [], dependsOn: [], networks: [], restart: 'unless-stopped' }
      ]
    });
  ...
  it('omits restart when set to no', () => {
    const out = buildCompose({
      projectName: 'myapp',
      services: [
        { name: 'web', image: 'nginx:alpine', command: '', ports: [], volumes: [], environment: [], dependsOn: [], networks: [], restart: 'no' }
      ]
    });
  ...
  it('emits depends_on as a list of names', () => {
    const out = buildCompose({
      projectName: 'myapp',
      services: [
        { name: 'web', image: 'nginx:alpine', command: '', ports: [], volumes: [], environment: [], dependsOn: ['api', 'db'], networks: [], restart: 'unless-stopped' },
        { name: 'api', image: 'node:20-alpine', command: '', ports: [], volumes: [], environment: [], dependsOn: [], networks: [], restart: 'unless-stopped' },
        { name: 'db', image: 'postgres:16-alpine', command: '', ports: [], volumes: [], environment: [], dependsOn: [], networks: [], restart: 'unless-stopped' }
      ]
    });
  ...
  it('emits environment as a list of KEY=VALUE strings', () => {
    const out = buildCompose({
      projectName: 'myapp',
      services: [
        { name: 'db', image: 'postgres:16-alpine', command: '', ports: [], volumes: [], environment: ['POSTGRES_PASSWORD=changeme', 'POSTGRES_DB=app'], dependsOn: [], networks: [], restart: 'unless-stopped' }
      ]
    });
  ...
  it('skips services with empty names', () => {
    const out = buildCompose({
      projectName: '',
      services: [
        { name: '', image: 'nginx:alpine', command: '', ports: [], volumes: [], environment: [], dependsOn: [], networks: [], restart: 'unless-stopped' },
        { name: 'web', image: 'nginx:alpine', command: '', ports: [], volumes: [], environment: [], dependsOn: [], networks: [], restart: 'unless-stopped' }
      ]
    });
    expect(out).toContain('web:');
  ...
  it('skips empty list entries', () => {
    const out = buildCompose({
      projectName: 'myapp',
      services: [
        { name: 'web', image: 'nginx:alpine', command: '', ports: ['80:80', '', '  '], volumes: ['./data:/data', ''], environment: ['A=1', ''], dependsOn: [], networks: [], restart: 'unless-stopped' }
      ]
    });
    expect(out).toContain('- "80:80"');
    ...
    const out = buildCompose({
      ...
      services: [
        ...
        },
        {
          ...
          ports: ['3000:3000'],
          volumes: [],
          environment: ['NODE_ENV=production', 'DATABASE_URL=postgres://postgres:changeme@db:5432/app'],
          dependsOn: ['db'],
          networks: [],
          restart: 'unless-stopped'
        },
