...
function build(overrides = {}) {
  ...
  const calculate = overrides.calculate || vi.fn(() => ({
...
describe('createFinanceCalculator', () => {
  it('renders the header with icon, title, and description', () => {
    ...
    expect(container.querySelector('.tool-description').textContent).toBe('A test finance calculator.');
  });
  ...
  it('shows the result area after calculation', () => {
    const { container } = build();
    container.querySelector('#test-finance-calc-btn').click();
    expect(container.querySelector('#test-finance-result').classList.contains('hidden')).toBe(false);
  });
