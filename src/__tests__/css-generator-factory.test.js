...
function build(overrides = {}) {
  ...
  const onUpdate = overrides.onUpdate || vi.fn(({ cssOutput }) => { cssOutput.textContent = 'border-radius: 0;'; });
...
describe('createCssGenerator', () => {
  ...
  it('copyBtn writes cssOutput text to clipboard and shows "Copied!"', async () => {
    const onUpdate = ({ cssOutput }) => { cssOutput.textContent = 'border-radius: 20px;'; };
  ...
  it('accepts a custom preview element', () => {
    const { container } = build({ previewHTML: '<div id="my-preview" class="custom-preview"></div>' });
  ...
  it('fires onUpdate on select change too', () => {
    const { container, onUpdate } = build({
      controlsHTML: '<div class="control-row"><select id="shape"><option value="a">A</option><option value="b">B</option></select></div>',
      onUpdate: vi.fn()
    });
    onUpdate.mockClear();
