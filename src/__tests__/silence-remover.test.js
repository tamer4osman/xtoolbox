...
import {
  ...
  buildTrimSegments,
  buildSelectFilter,
  buildAudioSelectFilter
} from "../tools/video/silence-remover.js";
...
describe("silence-remover", () => {
  describe("parseSilenceEvents", () => {
    ...
    it("handles unclosed silence (no end)", () => {
      const log = ["[silencedetect @ 0x1] silence_start: 7.0"].join("\n");
  ...
  describe("buildSelectFilter", () => {
    ...
    it("builds a select expression for multiple segments", () => {
      ...
      expect(filter).toBe(
        "select='between(t,0.000,2.000)+between(t,5.000,10.000)',setpts=N/FRAME_RATE/TB"
      );
    });
  });
  ...
  describe("buildAudioSelectFilter", () => {
    ...
    it("builds an aselect expression for multiple segments", () => {
      ...
      expect(filter).toBe(
        "aselect='between(t,0.000,2.000)+between(t,5.000,10.000)',asetpts=N/SR/TB"
      );
    });
  });
});
