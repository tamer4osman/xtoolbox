...
describe('nginx-config-generator', () => {
  ...
  describe('buildLocationBlocks', () => {
    ...
    it('emits a fastcgi block when PHP enabled', () => {
      const blocks = buildLocationBlocks({ enablePhp: true, phpSocket: 'unix:/run/php/php8.2.sock' });
    ...
    it('adds WebSocket upgrade headers when proxyWebsockets is true', () => {
      const blocks = buildLocationBlocks({ enableProxy: true, proxyUrl: 'http://x', proxyWebsockets: true });
      const joined = blocks.join('\n');
    ...
    it('does not add WebSocket headers when proxyWebsockets is false', () => {
      const blocks = buildLocationBlocks({ enableProxy: true, proxyUrl: 'http://x', proxyWebsockets: false });
  ...
  describe('buildServerBlock', () => {
    it('emits server_name, root, index, listen directives', () => {
      const out = buildServerBlock({ serverName: 'example.com', root: '/var/www/html', index: 'index.html', listen: 80 });
    ...
    it('adds an HTTP->HTTPS redirect server when enabled', () => {
      const out = buildServerBlock({ serverName: 'example.com', enableHttps: true, enableHttpRedirect: true });
    ...
    it('adds rate limiting when enabled', () => {
      const out = buildServerBlock({ serverName: 'x', enableRateLimit: true, rateLimitRate: '5r/s', rateLimitBurst: '10' });
