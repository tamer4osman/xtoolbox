  ...
  return {
    ...
    render(container) {
      ...
      container.querySelector("#enc-input").addEventListener("input", () => {
        try {
          container.querySelector("#enc-output").textContent = config.encode(
            container.querySelector("#enc-input").value
          );
        } catch {
          container.querySelector("#enc-output").textContent = "Invalid input";
        }
      });

      container.querySelector("#dec-input").addEventListener("input", () => {
        try {
          container.querySelector("#dec-output").textContent = config.decode(
            container.querySelector("#dec-input").value
          );
        } catch {
          container.querySelector("#dec-output").textContent = "Invalid input";
        }
      });
