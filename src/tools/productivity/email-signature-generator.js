...
function buildSignatureHtml(state) {
  ...
  const logoHtml = state.logoDataUrl && state.showLogo
  ...
  const socialHtml = socialLinks.length > 0
    ...
    : '';
  const addressHtml = state.address && state.showAddress
  ...
  if (photoHtml) {
    bodyRows.push(`<tr><td style="padding:0;"><table cellpadding="0" cellspacing="0" border="0"><tr>${photoHtml}<td style="padding:0;vertical-align:top;">${header}${contactHtml}${socialHtml}${addressHtml}</td></tr></table></td></tr>`);
  } else {
...
export const toolConfig = {
  ...
  description: 'Create professional email signatures with photo, social links, and custom styling. Preview live and copy HTML for Gmail, Outlook, and more.',
  icon: '✉️',
  accept: '.jpg,.jpeg,.png,.gif,.webp',
  maxSizeMB: 2,
  keywords: ['email', 'signature', 'professional', 'html', 'gmail', 'outlook', 'template', 'business'],
  steps: ['Fill in your personal and company details', 'Upload photo, logo, or banner image (optional)', 'Customize colors to match your brand', 'Preview the live signature on the right', 'Copy HTML or download to use in your email client'],
  faqs: [
    { question: 'How do I add this to Gmail?', answer: 'Copy the HTML, then go to Gmail Settings > See all settings > Signature. Paste the HTML using the "Insert signature" option in rich text mode.' },
    { question: 'How do I add this to Outlook?', answer: 'Copy the HTML, then go to Outlook > Settings > Compose and reply > Email signature. Create a new signature and paste the HTML.' },
    { question: 'Are my images embedded in the signature?', answer: 'Yes! Images are converted to data URLs and embedded directly in the HTML, so you don\'t need external hosting.' },
    { question: 'Will it work on mobile?', answer: 'Yes. The signature uses responsive inline styles that render well on all devices and dark mode.' },
  ]
};
...
export function render(container) {
  ...
  function updatePreview() {
    ...
    previewEl.innerHTML = html || '<div style="padding:40px;text-align:center;color:#999;font-size:13px;">Fill in your details to see the preview</div>';
    saveState(state);
  }
  ...
  function showToast(msg) {
    ...
    window._sigToastTimer = setTimeout(() => { toastEl.style.display = 'none'; }, 2500);
  }

  function readFileAsDataUrl(file) {
    return new Promise((resolve) => {
      if (!file) { resolve(null); return; }
      if (file.size > 2 * 1024 * 1024) { showToast('Image too large (max 2MB)'); resolve(null); return; }
  ...
  [['sig-text-color','textColor'],['sig-accent-color','accentColor'],['sig-link-color','linkColor']].forEach(([id, key]) => {
    const el = container.querySelector('#' + id);
    el.addEventListener('input', () => {
      state[key] = el.value;
      updatePreview();
  ...
  container.querySelector('#sig-copy').addEventListener('click', async () => {
    const html = buildSignatureHtml(state);
    if (!html) { showToast('Fill in some details first'); return; }
    try {
