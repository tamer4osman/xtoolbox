...
export const toolConfig = {
  ...
  description:
    "Adjust SRT and VTT subtitle timestamps client-side with millisecond accuracy sync offsets.",
  icon: "⏱️",
  accept: ".srt,.vtt",
  maxSizeMB: 10,
  ...
  steps: [
    ...
    "Enter offset in milliseconds (positive = delay, negative = advance)",
    "Preview the timestamp changes",
    "Download the adjusted subtitle file"
  ],
  faqs: [
    {
      question: "What subtitle formats are supported?",
      answer: "SRT (SubRip) and VTT (WebVTT) subtitle formats are supported."
    },
    {
      question: "What does a positive offset do?",
      answer:
        "A positive offset delays subtitles (makes them appear later). A negative offset advances them (makes them appear earlier)."
    },
    {
      question: "Will this change the subtitle text?",
      answer: "No, only the timestamps are adjusted. The subtitle text remains unchanged."
    }
  ]
};
...
export function render(container) {
  ...
  for (let i = 0; i < presets.length; i++) {
    container
      .querySelector(`#${presetIds[i]}`)
      .addEventListener("click", () =>
        setOffset(presetIds[i].includes("n") ? -Math.abs(presets[i]) : presets[i])
      );
  }
