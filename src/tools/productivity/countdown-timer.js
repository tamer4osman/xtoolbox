...
export const toolConfig = {
  ...
  description:
    "Count down to a specific date or event. Save multiple countdowns and see live updates.",
  icon: "⏳",
  accept: null,
  maxSizeMB: null,
  ...
  faqs: [
    {
      question: "Does it save my countdowns?",
      answer: "Yes! Countdowns are saved in your browser and persist across page loads."
    },
    {
      question: "What happens when time runs out?",
      answer: 'The countdown shows "Time\'s up!" but remains visible.'
    }
  ]
};
...
export function render(container) {
  ...
  function renderAll() {
    ...
    if (countdowns.length === 0) {
      list.innerHTML =
        '<div style="text-align:center;padding:var(--space-8);color:var(--color-text-muted);">No countdowns yet. Add one above!</div>';
      return;
    }
    ...
    for (let i = 0; i < countdowns.length; i++) {
      ...
      card.style.cssText =
        "border:1px solid var(--color-border);border-radius:var(--radius-md);padding:var(--space-4);margin-bottom:var(--space-3);";
      card.innerHTML = `
