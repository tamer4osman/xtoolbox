...
export function render(container) {
  ...
  const upload = createFileUpload({
    ...
    onFilesSelected: async (selectedFiles) => {
      ...
      totalFiles.textContent = files.length + ' WebP file(s)';
  ...
  convertBtn.addEventListener('click', async () => {
    ...
    try {
      for (let i = 0; i < images.length; i++) {
        ...
        const fileName = files[i].name.replace(/\.webp$/i, '');
        downloadBlob(blob, `${fileName}.jpg`);
      }
      
      showToast({ message: `Converted ${images.length} WebP(s) to JPG!`, type: 'success' });
    } catch (err) {
