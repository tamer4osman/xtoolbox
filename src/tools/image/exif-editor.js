...
const FIELDS = {
  '0th': {
    ...
    306: 'DateTime', 33432: 'Copyright'
  },
  'Exif': {
    33434: 'ExposureTime', 33437: 'FNumber', 36867: 'DateTimeOriginal',
    36868: 'DateTimeDigitized', 37377: 'ShutterSpeedValue', 37378: 'ApertureValue',
    37380: 'ExposureBiasValue', 37381: 'MaxApertureValue', 37383: 'MeteringMode',
    37385: 'Flash', 37386: 'FocalLength', 41486: 'FocalPlaneXResolution',
    41488: 'FocalPlaneResolutionUnit', 42016: 'ImageUniqueID',
    42036: 'LensModel', 42037: 'LensSerialNumber'
  },
  'GPS': {
    1: 'GPSVersionID', 2: 'GPSLatitudeRef', 3: 'GPSLatitude',
    4: 'GPSLongitudeRef', 5: 'GPSLongitude', 6: 'GPSAltitudeRef',
    7: 'GPSAltitude', 29: 'GPSDateStamp'
  }
};
...
export const toolConfig = {
  ...
  faqs: [
    { question: 'What EXIF fields can I edit?', answer: 'Camera make/model, date/time, GPS coordinates, exposure settings, lens info, copyright, and more.' },
    { question: 'What image formats are supported?', answer: 'JPEG only. PNG, HEIF, and WebP use different metadata formats not supported by this tool.' }
  ]
};
...
function parseInput(ifd, tag, raw) {
  ...
  if (tag === 33434 || tag === 33437 || tag === 37377 || tag === 37378 || tag === 37380 || tag === 37381 || tag === 37386 || tag === 41486) {
    if (val.includes('/')) {
...
export function render(container) {
  ...
  const upload = createFileUpload({
    ...
    onFilesSelected: async (files) => {
      ...
      editorArea.innerHTML = '<div style="text-align:center;padding:var(--space-4);"><div class="spinner"></div><p>Reading EXIF data...</p></div>';
      ...
      try {
        ...
        if (sections.length === 0) {
          editorArea.innerHTML = '<div style="text-align:center;padding:var(--space-8);color:var(--color-text-muted);">No EXIF data found in this image.</div>';
          return;
        }
        ...
        editorArea.querySelector('#exif-save').addEventListener('click', async () => {
          try {
            ...
            const modified = { '0th': { ...currentExifObj['0th'] }, 'Exif': { ...currentExifObj['Exif'] }, 'GPS': { ...currentExifObj['GPS'] } };
            ...
            Object.keys(modified['0th']).forEach(k => { if (modified['0th'][k] === undefined) delete modified['0th'][k]; });
            Object.keys(modified['Exif']).forEach(k => { if (modified['Exif'][k] === undefined) delete modified['Exif'][k]; });
            Object.keys(modified['GPS']).forEach(k => { if (modified['GPS'][k] === undefined) delete modified['GPS'][k]; });
