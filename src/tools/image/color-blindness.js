...
export const toolConfig = {
  ...
  category: 'image',
  description: 'Upload an image and apply color blindness simulations: deuteranopia, protanopia, tritanopia, and more.',
  icon: '👁️',
  accept: 'image/*',
  maxSizeMB: 50,
  keywords: ['color blindness', 'deuteranopia', 'protanopia', 'tritanopia', 'simulator', 'vision', 'accessibility'],
  steps: ['Upload an image', 'Click a simulation type to preview', 'Compare original vs simulated using the slider', 'Download the simulated image'],
  faqs: [
    { question: 'What is color blindness simulation?', answer: 'It applies mathematical color matrices to approximate how someone with color vision deficiency perceives images.' },
    { question: 'Are the simulations 100% accurate?', answer: 'They are approximations based on standard models. Real perception varies by individual.' },
    { question: 'Why simulate color blindness?', answer: 'Designers and developers use it to ensure their content is accessible to people with color vision deficiencies.' }
  ]
};
...
export function render(container) {
  ...
  function renderTypeGrid() {
    ...
    SIMULATION_TYPES.forEach(t => {
      ...
      item.style.cssText = 'cursor:pointer;border-radius:var(--radius-md);padding:var(--space-3);border:2px solid transparent;transition:border-color .2s;text-align:center;background:var(--color-surface);';
      ...
      desc.style.cssText = 'font-size:var(--text-xs);color:var(--color-text-secondary);margin-top:var(--space-1);';
