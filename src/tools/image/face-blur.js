...
export const toolConfig = {
  ...
  steps: ['Upload an image', 'Choose blur or pixelate method', 'Adjust intensity', 'Download anonymized image'],
  faqs: [
    { question: 'How accurate is face detection?', answer: 'BlazeFace detects faces reliably for photos with clearly visible faces. Side-profile or partially obscured faces may be missed.' },
    { question: 'Is the image sent anywhere?', answer: 'No. All processing happens locally in your browser.' }
  ]
};
...
export function render(container) {
  ...
  async function detectFaces() {
    ...
    try {
      const { env } = await import('https://cdn.jsdelivr.net/npm/onnxruntime-web@1.17.0/dist/onnxruntime-web.min.js');
  ...
  processBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      showToast({ message: `Face detection failed: ${err.message}. Trying fallback...`, type: 'warning' });
      const canvas = previewCanvas;
      const ctx = canvas.getContext('2d');
      canvas.width = originalImage.width;
      canvas.height = originalImage.height;
