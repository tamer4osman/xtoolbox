...
export const toolConfig = {
  ...
  name: 'Image Splitter',
  category: 'image',
  description: 'Split any image into a grid of equal-sized tiles. Perfect for Instagram carousel posts, photo grids, or creating puzzle-like image collections.',
  icon: '✂️',
  accept: 'image/*',
  maxSizeMB: 50,
  keywords: ['split image', 'image splitter', 'instagram grid'],
  steps: ['Upload an image', 'Set number of rows and columns', 'Preview how the image will be split', 'Download all tiles as ZIP'],
  faqs: [
    { question: 'How does the splitting work?', answer: 'The image is divided into equal tiles based on rows × columns. For example, a 3×3 grid splits your image into 9 equal parts.' },
    { question: 'What happens if image dimensions don\'t divide evenly?', answer: 'Each tile gets the floor value of the division. Any remaining pixels on the right/bottom edges are cropped.' },
    { question: 'What format are the tiles?', answer: 'All tiles are saved as PNG to preserve quality. They\'re named with row_col format (e.g., 1_1.png, 1_2.png).' },
    { question: 'How do I use for Instagram?', answer: 'Split your image into 3×1 for horizontal carousel, or 3×3 for a 9-image grid. Upload tiles in order (left to right, top to bottom).' }
  ]
};
...
export function render(container) {
  ...
  function updatePreview() {
    ...
    ctx.lineWidth = 1;
    for (let r = 1; r < rows; r++) { ctx.beginPath(); ctx.moveTo(0, r * canvas.height / rows); ctx.lineTo(canvas.width, r * canvas.height / rows); ctx.stroke(); }
    for (let c = 1; c < cols; c++) { ctx.beginPath(); ctx.moveTo(c * canvas.width / cols, 0); ctx.lineTo(c * canvas.width / cols, canvas.height); ctx.stroke(); }
