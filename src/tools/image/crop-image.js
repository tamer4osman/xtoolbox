...
export const toolConfig = {
  ...
  steps: [
    ...
    "Choose aspect ratio preset (optional)",
    "Download cropped image"
  ],
  faqs: [
    {
      question: "Can I crop to a specific aspect ratio?",
      answer: "Yes. Use the preset buttons for common ratios like 1:1, 4:3, 16:9."
    }
  ]
};
...
function initCropper(
  ...
  selection.addEventListener("mousedown", e => {
    ...
    selectionStartX = Math.min(cropStart.x, cropEnd.x);
    selectionStartY = Math.min(cropStart.y, cropEnd.y);
    isMoving = true;
    setCropState({ isDragging: true });
  });
  ...
  document.addEventListener("mousemove", e => {
    ...
    if (isMoving && cropStart && cropEnd) {
      ...
      setCropState({
        cropEnd: {
          x: Math.max(0, Math.min(e.clientX - rect.left, w)),
          y: Math.max(0, Math.min(e.clientY - rect.top, h))
        }
      });
    }
    const cs = getCropState();
    if (cs.cropStart && cs.cropEnd)
      updateSelection(cropContainer, cs.cropStart, cs.cropEnd, cs.currentRatio, w, h);
  });
  ...
  container.querySelectorAll("[data-ratio]").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      updateSelection(
        cropContainer,
        getCropState().cropStart,
        getCropState().cropEnd,
        currentRatio,
        scaledW,
        scaledH
      );
...
export function render(container) {
    ...
    onFilesSelected: async files => {
      ...
      originalImg.onload = () => {
        ...
        initCropper(
          ...
          cropInfo,
          () => cropState,
          patch => Object.assign(cropState, patch)
        );
      };
  ...
  cropBtn.addEventListener("click", () => {
    ...
    if (sw < 1 || sh < 1) {
      showToast({ message: "Select a larger area", type: "warning" });
      return;
    }
