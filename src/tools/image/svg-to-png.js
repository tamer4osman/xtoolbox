...
export const toolConfig = {
  ...
  steps: [
    "Upload SVG image(s)",
    "Set output size and background",
    'Click "Convert to PNG"',
    "Download converted images"
  ],
  faqs: [
    {
      question: "Will the PNG be high quality?",
      answer:
        "Yes, you can set the output resolution up to 4x the original SVG size for crisp results."
    },
    {
      question: "What happens to transparency?",
      answer:
        "SVG transparency is preserved by default. You can set a solid background color if needed."
    },
    {
      question: "Can I convert multiple SVGs?",
      answer: "Yes, upload multiple SVGs and they will all be converted."
    }
  ]
};
    ...
    async onConvert({ files, uploadedData, progress }) {
      ...
      for (let i = 0; i < svgs.length; i++) {
        ...
        const url = URL.createObjectURL(
          new Blob([svgs[i]], { type: "image/svg+xml;charset=utf-8" })
        );
        ...
        downloadBlob(
          ...
          files[i].name.replace(/\.svg$/i, "") + ".png"
        );
      }
