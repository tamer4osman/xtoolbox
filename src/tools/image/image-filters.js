...
export const toolConfig = {
  ...
  steps: [
    ...
    "Click a filter to preview on your image",
    "Download your filtered image"
  ],
  faqs: [
    {
      question: "Can I apply multiple filters?",
      answer: "Each filter is a preset. Apply one at a time by clicking its thumbnail."
    },
    {
      question: "Are the filters reversible?",
      answer: 'Yes — click "Normal" or re-upload to restore the original image.'
    },
    {
...
export function render(container) {
  ...
  function renderFilterGrid() {
    ...
    FILTERS.forEach(f => {
      ...
      item.style.cssText =
        "cursor:pointer;border-radius:var(--radius-md);overflow:hidden;border:2px solid transparent;transition:border-color .2s;";
      ...
      tCtx.drawImage(
        ...
        dx,
        dy,
        originalImg.naturalWidth * s,
        originalImg.naturalHeight * s
      );
      ...
      label.style.cssText =
        "padding:var(--space-2);text-align:center;font-size:var(--text-sm);font-weight:600;background:var(--color-surface);";
