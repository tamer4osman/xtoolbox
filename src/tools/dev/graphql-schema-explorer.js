...
export const toolConfig = {
  ...
  steps: ['Upload or paste a GraphQL SDL schema', 'Browse types, queries, mutations', 'View field details and relationships'],
  faqs: [
    { question: 'What is SDL?', answer: 'Schema Definition Language is the standard way to define GraphQL schemas.' },
    { question: 'Does it validate schemas?', answer: 'No, it parses and displays SDL for browsing only.' }
  ]
};
...
export function render(container) {
  ...
  const upload = createFileUpload({
    ...
    onFilesSelected: async (files) => {
      ...
      try {
        const sdl = await readFileAsText(files[0]);
        types = parseSDL(sdl);
        renderTree();
  ...
  function renderTree() {
    ...
    function renderTypeCard(type) {
      const fieldsHtml = type.fields.map(f => {
        const typeBadge = f.deprecated ? '<span style="color:var(--color-error);font-size:var(--text-xs);margin-left:4px;">DEPRECATED</span>' : '';
        const argsHtml = f.args.length > 0
        ...
        const typeLink = isRef ? `<span style="color:var(--color-primary);cursor:pointer;" class="type-link" data-type="${f.type.replace(/[[\]!]/g, '')}">${f.type}</span>` : `<code>${f.type}</code>`;
        return `
