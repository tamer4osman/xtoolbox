...
export const toolConfig = {
  ...
  category: "dev",
  description:
    "Generate formatted local network alias hosts configurations from a simple visual key table.",
  icon: "🌐",
  accept: null,
  maxSizeMB: null,
  keywords: [
  ...
  steps: [
    "Add IP and hostname pairs",
    "Choose a template or create custom entries",
    "Copy or download the generated hosts file content"
  ],
  faqs: [
    {
      question: "What is a hosts file?",
      answer:
        "The hosts file is a local system file that maps hostnames to IP addresses. It overrides DNS resolution for the specified hostnames."
    },
    {
      question: "Where is the hosts file located?",
      answer: "Linux/macOS: /etc/hosts. Windows: C:\\Windows\\System32\\drivers\\etc\\hosts."
    },
    {
      question: "How do I apply changes?",
      answer:
        "After modifying the hosts file, flush your DNS cache. On Windows: ipconfig /flushdns. On macOS: sudo dscacheutil -flushcache. On Linux: sudo systemd-resolve --flush-caches."
    }
  ]
};
...
export function render(container) {
  ...
  function createEntryRow(ip = "", hostname = "", comment = "") {
    ...
    row.style.cssText =
      "display:grid;grid-template-columns:1fr 1fr 1fr auto;gap:var(--space-2);margin-bottom:var(--space-2);align-items:center;";
    row.innerHTML = `
