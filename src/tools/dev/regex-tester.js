export const toolConfig = {
  ...
  category: 'dev',
  description: 'Test regular expressions with live matching, capture groups, visual railroad diagram, and plain English explanation.',
  icon: '🔍',
  ...
  while (i < pattern.length) {
    ...
    if (ch === '\\') {
      ...
      if (pattern[i + 1] === '?') {
        ...
        else if (pattern[i + 2] === '<' && pattern[i + 3] === '=') { groupType = 'lookbehind'; value = '(?<='; i += 4; }
        else if (pattern[i + 2] === '<' && pattern[i + 3] === '!') { groupType = 'neglookbehind'; value = '(?<!'; i += 4; }
        else { i += 2; }
      } else {
        i++;
      }
...
export function highlightMatches(text, regex) {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    escaped.push('<mark style="background:' + color + ';padding:1px 2px;border-radius:2px;border:1px solid rgba(0,0,0,0.1)">' + escapeHtml(match[0]) + '</mark>');
...
function buildRailroadSVG(tokens) {
  ...
  function layoutTokens(tokens) {
    ...
    while (i < tokens.length) {
      ...
      if (tok.type === 'group-start') {
        ...
        const groupLabel = tok.groupType === 'noncapture' ? '?:' :
  ...
  function renderItems(items, svg, x, y) {
    for (let idx = 0; idx < items.length; idx++) {
      ...
      if (item.kind === 'node') {
        ...
        svg += '<rect x="' + x + '" y="' + (y - NODE_H / 2 - 6) + '" width="' + boxW + '" height="' + (NODE_H + 12) + '" rx="6" fill="none" stroke="' + item.color + '" stroke-width="1.5" stroke-dasharray="5,3"/>';
        if (item.label) {
      ...
      if (idx < items.length - 1 && item.kind !== 'branch') {
        svg += '<line x1="' + (x - H_GAP) + '" y1="' + y + '" x2="' + x + '" y2="' + y + '" stroke="#9ca3af" stroke-width="2"/>';
      }
    }
    return svg;
  }
  ...
  svg += '<line x1="' + endX + '" y1="' + y + '" x2="' + (endX + 10) + '" y2="' + y + '" stroke="#9ca3af" stroke-width="2"/>';
...
export function render(container) {
  ...
  function run() {
    ...
    try {
      ...
      tabContent.innerHTML = '<span style="color:#ef4444;font-size:13px">Invalid regex: ' + escapeHtml(e.message) + '</span>';
    ...
    if (activeTab === 'diagram') {
      try {
        const tokens = tokenizeRegex(pattern);
        tabContent.innerHTML = buildRailroadSVG(tokens) || '<span class="rt-no-data">Empty pattern</span>';
      } catch {
        tabContent.innerHTML = '<span style="color:#ef4444;font-size:13px">Cannot render diagram</span>';
      }
      ...
      if (matches.length === 0) {
        ...
        let html = '<div class="rt-match-count">' + matches.length + ' match' + (matches.length !== 1 ? 'es' : '') + '</div>';
        matches.forEach((m, i) => {
          html += '<div class="rt-match-item"><span class="rt-match-idx">#' + (i + 1) + '</span><span class="rt-match-val">' + escapeHtml(m.value) + '</span><span class="rt-match-pos">pos ' + m.position + '</span></div>';
        });
      ...
      if (groupMatches.length === 0) {
        ...
        if (maxGroups === 0) {
          tabContent.innerHTML = '<span class="rt-no-data">No capturing groups in this pattern</span>';
        } else {
          ...
          groupMatches.forEach((m, i) => {
            ...
            for (let g = 0; g < maxGroups; g++) {
              ...
              html += '<td>' + (val !== '' ? '"' + escapeHtml(val) + '"' : '<span style="color:#ccc">-</span>') + '</td>';
            }
            html += '</tr>';
          });
          html += '</tbody></table>';
