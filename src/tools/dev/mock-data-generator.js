export const toolConfig = {
  ...
  category: 'dev',
  description: 'Generate realistic fake data (names, emails, addresses) in JSON, CSV, SQL, and TypeScript formats.',
  icon: '🎲',
...
const mockData = {
  ...
  streets: ['Main St', 'Oak Ave', 'Maple Dr', 'Cedar Ln', 'Pine St', 'Elm St', 'Washington Blvd', 'Lake View', 'Park Ave', 'Center St'],
  cities: ['New York', 'Los Angeles', 'Chicago', 'Houston', 'Phoenix', 'Philadelphia', 'San Antonio', 'San Diego', 'Dallas', 'San Jose'],
  countries: ['United States', 'Canada', 'United Kingdom', 'Germany', 'France', 'Australia', 'Japan', 'Brazil', 'India', 'Mexico'],
  companies: ['Acme Corp', 'GlobalTech', 'Innovate LLC', 'TechVision', 'DataSync', 'CloudNine', 'ByteWorks', 'CodeMasters', 'DigitalEdge', 'InfoTech'],
  jobTitles: ['Software Engineer', 'Product Manager', 'Designer', 'Data Analyst', 'Marketing Manager', 'Sales Representative', 'Project Manager', 'Developer', 'Consultant', 'Director'],
...
function generateField(key) {
  switch (key) {
    case 'firstName': return random(mockData.firstNames);
    case 'lastName': return random(mockData.lastNames);
    case 'email': return `${random(mockData.firstNames).toLowerCase()}.${random(mockData.lastNames).toLowerCase()}${randomInt(1, 99)}@${random(mockData.domains)}`;
    case 'phone': return `(${randomInt(200, 999)}) ${randomInt(100, 999)}-${randomInt(1000, 9999)}`;
    case 'address': return `${randomInt(1, 9999)} ${random(mockData.streets)}`;
    case 'city': return random(mockData.cities);
    case 'country': return random(mockData.countries);
    case 'company': return random(mockData.companies);
    case 'jobTitle': return random(mockData.jobTitles);
    case 'creditCard': return `**** **** **** ${randomInt(1000, 9999)}`;
    case 'username': return `${random(mockData.firstNames).toLowerCase()}${randomInt(1, 99)}`;
    case 'password': return `Pass${randomInt(1000, 9999)}!`;
    default: return '';
  }
}
