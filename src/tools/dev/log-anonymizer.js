...
export function render(container) {
    ...
    <div class="anonymizer-container">
      <div class="input-section">
        <div class="form-group">
          <label for="log-input">Server Log Input</label>
          <textarea id="log-input" class="log-textarea" placeholder="Paste your server log here..."></textarea>
        </div>
        <div class="form-group">
          <label for="file-upload">Or upload a log file:</label>
          <input type="file" id="file-upload" accept=".log,.txt" class="file-input">
        </div>
      </div>
      ...
      <div class="options-section">
        ...
        <div class="checkbox-group">
          <label class="checkbox-label">
            <input type="checkbox" id="mask-ipv4" checked> IP Addresses (IPv4)
          </label>
          <label class="checkbox-label">
            <input type="checkbox" id="mask-ipv6" checked> IP Addresses (IPv6)
          </label>
          <label class="checkbox-label">
            <input type="checkbox" id="mask-email" checked> Email Addresses
          </label>
          <label class="checkbox-label">
            <input type="checkbox" id="mask-apikey" checked> API Keys & Tokens
          </label>
          <label class="checkbox-label">
            <input type="checkbox" id="mask-bearer" checked> Bearer Tokens
          </label>
          <label class="checkbox-label">
            <input type="checkbox" id="mask-db" checked> Database Connections
          </label>
          <label class="checkbox-label">
            <input type="checkbox" id="mask-credit" checked> Credit Card Numbers
          </label>
          <label class="checkbox-label">
            <input type="checkbox" id="mask-uuid" checked> UUIDs
          </label>
        </div>
      </div>
      ...
      <div class="output-section">
        ...
        <div class="form-group">
          <label for="log-output">Sanitized Output</label>
          <textarea id="log-output" class="log-textarea output" placeholder="Anonymized log will appear here..." readonly></textarea>
        </div>
      </div>
    </div>
  `;
  ...
  const anonymizeBtn = container.querySelector('#anonymize-btn');
  ...
  anonymizeBtn.addEventListener('click', () => {
    ...
    if (container.querySelector('#mask-ipv4').checked) enabledPatterns.push('ipv4');
    if (container.querySelector('#mask-ipv6').checked) enabledPatterns.push('ipv6');
    if (container.querySelector('#mask-email').checked) enabledPatterns.push('email');
    if (container.querySelector('#mask-apikey').checked) enabledPatterns.push('apiKey');
    if (container.querySelector('#mask-bearer').checked) enabledPatterns.push('bearerToken');
    if (container.querySelector('#mask-db').checked) enabledPatterns.push('dbConnection');
    if (container.querySelector('#mask-credit').checked) enabledPatterns.push('creditCard');
    if (container.querySelector('#mask-uuid').checked) enabledPatterns.push('uuid');

    const { result, found } = anonymizeLog(text, enabledPatterns);
    logOutput.value = result;

    const totalFound = Object.values(found).reduce((a, b) => a + b, 0);
    if (totalFound > 0) {
      stats.innerHTML = '<h4>Masked Items:</h4>' + 
