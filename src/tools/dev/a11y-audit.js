export const toolConfig = {
  ...
  category: 'dev',
  description: 'Analyze web pages for accessibility issues. Check WCAG compliance, ARIA labels, color contrast, and more.',
  icon: '♿',
...
function runAudit(html, url) {
  ...
  const hasLang = doc.documentElement.getAttribute('lang') || doc.documentElement.getAttribute('xml:lang');
  if (!hasLang) {
  ...
  images.forEach((img, i) => {
    ...
    if (!alt && role !== 'presentation' && role !== 'none') {
      ...
      results.failed.push({ check: 'alt-text', msg: 'Image has empty alt (should be presentation if decorative)' });
    }
  });
  ...
  links.forEach(link => {
    ...
    } else if (text && ['click here', 'read more', 'link', 'here', 'more'].includes(text.toLowerCase())) {
      results.warnings.push({ check: 'link-text', msg: `Link text "${text}" is not descriptive` });
    }
  });

  const buttons = body.querySelectorAll('button, input[type="button"], input[type="submit"], input[type="reset"]');
  buttons.forEach(btn => {
  ...
  tabIndex.forEach(el => {
    ...
    if (idx === '-1') {
      results.warnings.push({ check: 'focus-visible', msg: 'Element has tabindex="-1", may not be keyboard accessible' });
    }
  });

  const hasOutline = body.querySelectorAll('[style*="outline"]');
  results.warnings.push({ check: 'focus-visible', msg: 'Manual check: verify all interactive elements have visible focus' });

  if (hasLang) results.passed.push({ check: 'lang-attr', msg: 'Language attribute present' });

  const passedChecks = wcagChecks.map(c => c.id).filter(id => {
    return !results.failed.some(f => f.check === id) && !results.warnings.some(w => w.check === id);
  });
  passedChecks.forEach(id => {
    results.passed.push({ check: id, msg: wcagChecks.find(c => c.id === id).title + ' appears OK' });
  });
