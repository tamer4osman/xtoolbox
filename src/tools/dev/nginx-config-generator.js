...
import { NGX_CSS, escapeHtml, fieldHtml, selectHtml, checkHtml, NGX_FIELDS, bindNginxEvents } from './nginx-constants.js';
...
export function render(container) {
    ...
    <div class="tool-layout">
      <div class="ngx-shell">
        <div class="ngx-pane ngx-controls">
          ...
          <div class="ngx-section"><h3>WWW redirect</h3>${sh('enableWwwRedirect', 'Redirect', [{ value: 'none', label: 'No redirect' }, { value: 'to-www', label: 'Force www. prefix' }, { value: 'from-www', label: 'Force non-www' }])}</div>
  ...
  function syncConditional() {
    const set = (sel, show) => { const el = container.querySelector(sel); if (el) el.style.display = show ? '' : 'none'; };
  ...
  function syncFormFromState() {
    container.querySelectorAll('.ngx-toggle').forEach(el => { el.checked = !!state[el.dataset.field]; });
  ...
  bindNginxEvents({ container, getState, setState, resetState, syncFormFromState, syncConditional, generate, PRESETS, applyPreset, copyToClipboard, downloadBlob, showToast });
