...
export const toolConfig = {
  ...
  category: "dev",
  description:
    "Generate nginx.conf server blocks visually: HTTPS, reverse proxy, PHP-FPM, load balancing, rate limiting, security headers, and more. Includes ready-made presets for static sites, WordPress, Node.js, and load balancers.",
  icon: "🌐",
  accept: null,
  maxSizeMB: null,
  keywords: [
  ...
  faqs: [
    {
      question: "Where do I put the generated config?",
      answer:
        "On most Linux installs the main config is /etc/nginx/nginx.conf and per-site configs go in /etc/nginx/sites-available/ as files symlinked into sites-enabled/."
    },
    {
      question: "How do I test the config before reloading?",
      answer:
        'Run sudo nginx -t to validate the syntax. If it says "test is successful", reload with sudo nginx -s reload or sudo systemctl reload nginx.'
    },
    {
      question: "Do I need to escape anything in custom locations?",
      answer:
        "The generator outputs raw nginx directives. Make sure the path you type does not contain spaces; if it does, wrap the whole value in double quotes in the field."
    },
    {
      question: "What does the upstream block do?",
      answer:
        "An upstream block defines a pool of backend servers. With more than one backend, nginx round-robins requests between them, providing simple load balancing with no extra software."
    },
    {
      question: "Does this work with Lets Encrypt?",
      answer:
        "Yes. Enable HTTPS and put the Lets Encrypt fullchain.pem and privkey.pem paths into the SSL certificate and key fields. Run certbot to obtain the files first."
    }
  ]
};
...
export function buildServerBlock(state) {
  ...
  if (state.enableHttps) {
    lines.push("");
    buildHttpsBlock(state, listenExtra)
      .split("\n")
      .forEach(l => lines.push(l));
  }

  lines.push("");
  buildWwwRedirectBlock(state, listenExtra)
    .split("\n")
    .forEach(l => lines.push(l));

  lines.push("");
  buildHttpRedirectBlock(state, listenExtra)
    .split("\n")
    .forEach(l => lines.push(l));
    ...
    <div class="tool-layout">
      <div class="ngx-shell">
        <div class="ngx-pane ngx-controls">
            ...
            <div class="form-group">
              ...
              <select id="ngx-preset" class="text-input">
                ${Object.entries(PRESETS)
                  .map(
                    ([k, p]) =>
                      `<option value="${k}"${k === state.preset ? " selected" : ""}>${p.label}</option>`
                  )
                  .join("")}
              </select>
          ...
          <div class="ngx-section"><h3>Server</h3>${fh("listen", "Listen port", "80")}${fh("listenExtra", "Extra listen flags", "default_server")}${fh("serverName", "Server name", "example.com www.example.com")}${fh("root", "Root directory", "/var/www/html")}${fh("index", "Index files", "index.html index.htm")}</div>
          <div class="ngx-section"><h3>Logging</h3>${fh("accessLog", "Access log", "/var/log/nginx/access.log")}${fh("errorLog", "Error log", "/var/log/nginx/error.log")}</div>
          <div class="ngx-section"><h3>Performance</h3><div class="ngx-checks">${ch("enableGzip", "Enable gzip", "Compress text responses")}${ch("enableStaticCache", "Cache static assets", "expires + Cache-Control headers")}</div><div id="ngx-cacheExpires-wrap" style="display:none;">${fh("cacheExpires", "Expires (e.g. 30d, 12h)", "30d")}</div></div>
          <div class="ngx-section"><h3>Security</h3><div class="ngx-checks">${ch("enableSecurityHeaders", "Add security headers", "XFO, nosniff, Referrer-Policy, Permissions-Policy")}</div><h4>Rate limiting</h4><div class="ngx-checks">${ch("enableRateLimit", "Enable rate limiting", "limit_req in server block")}</div><div id="ngx-rate-opts" style="display:none;">${fh("rateLimitRate", "Rate", "10r/s")}${fh("rateLimitBurst", "Burst", "20")}</div></div>
          <div class="ngx-section"><h3>HTTPS / TLS</h3><div class="ngx-checks">${ch("enableHttps", "Enable HTTPS server block (port 443)")}${ch("enableHttpRedirect", "Redirect HTTP → HTTPS")}</div><div id="ngx-https-opts" style="display:none;">${fh("sslCertificate", "ssl_certificate", "/etc/letsencrypt/live/example.com/fullchain.pem")}${fh("sslCertificateKey", "ssl_certificate_key", "/etc/letsencrypt/live/example.com/privkey.pem")}${fh("sslProtocols", "ssl_protocols", "TLSv1.2 TLSv1.3")}</div></div>
          <div class="ngx-section"><h3>WWW redirect</h3>${sh("enableWwwRedirect", "Redirect", [
            { value: "none", label: "No redirect" },
            { value: "to-www", label: "Force www. prefix" },
            { value: "from-www", label: "Force non-www" }
          ])}</div>
          <div class="ngx-section"><h3>PHP-FPM</h3><div class="ngx-checks">${ch("enablePhp", "Pass .php to PHP-FPM", "Adds a fastcgi location block")}</div><div id="ngx-php-opts" style="display:none;">${fh("phpSocket", "FPM socket / address", "unix:/run/php/php8.2-fpm.sock")}</div></div>
          <div class="ngx-section"><h3>Reverse proxy</h3><div class="ngx-checks">${ch("enableProxy", "Enable reverse proxy", "Adds a location / with proxy_pass")}</div><div id="ngx-proxy-opts" style="display:none;">${fh("proxyUrl", "Proxy pass URL", "http://127.0.0.1:3000")}<div class="ngx-checks">${ch("proxyWebsockets", "WebSocket support", "Adds Upgrade/Connection headers")}</div></div></div>
          <div class="ngx-section"><h3>Load balancing (upstream)</h3><div class="ngx-checks">${ch("enableUpstream", "Define upstream block", "Round-robins between backends")}</div><div id="ngx-upstream-opts" style="display:none;">${fh("upstreamName", "Upstream name", "backend")}${fh("upstreamBackends", "Backends (one per line)", "http://127.0.0.1:8080")}<small class="ngx-hint">Use the upstream name (e.g. <code>http://backend</code>) as the proxy URL above to route to this pool.</small></div></div>
        </div>
        <div class="ngx-pane ngx-output">
          <div class="ngx-output-header"><div class="ngx-meta"><span class="ngx-badge" id="ngx-line-count">0 lines</span><span class="ngx-badge ngx-badge-soft" id="ngx-byte-count">0 B</span></div><div class="ngx-actions"><button class="btn btn-sm" id="ngx-reset" type="button">Reset</button><button class="btn btn-sm" id="ngx-download" type="button">Download</button><button class="btn btn-primary btn-sm" id="ngx-copy" type="button">Copy</button></div></div>
          <pre class="ngx-code" id="ngx-output" aria-live="polite"></pre>
          <div class="ngx-foot"><small>Always validate with <code>sudo nginx -t</code> before reloading.</small></div>
        </div>
      </div>
    </div>
  `;
