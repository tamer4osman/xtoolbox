...
export const toolConfig = {
  ...
  category: "dev",
  description:
    "Estimate token counts and per-model cost for OpenAI GPT-4o / GPT-4 / GPT-3.5, Anthropic Claude 3.5 / 3 Opus, and Google Gemini 1.5 models. Auto-detects prose vs. code content and applies a calibrated heuristic. Includes a side-by-side model comparison table and copyable stats.",
  icon: "🧮",
  accept: null,
  maxSizeMB: null,
  keywords: [
  ...
  faqs: [
    {
      ...
      answer:
        "It uses a calibrated heuristic, not the actual BPE tokenizer. For English prose the error is typically under 10%. For code, CJK, and other dense content the error can be higher. The estimate is clearly labeled as such — for exact counts, run the same text through the official tiktoken library or your model provider's tokenizer tool."
    },
    {
      question: "Why estimate instead of using tiktoken?",
      answer:
        "A real BPE tokenizer ships 1–2 MB of vocabulary data and a WASM core. For a fast in-browser estimator that works for the common case, the heuristic avoids the 2 MB download and runs instantly on any text length."
    },
    {
      question: "How is the content type detected?",
      answer:
        "The counter counts CJK characters, code-like characters (braces, semicolons, operators), and whitespace. If CJK is over 10% of the text it picks the CJK ratio; if code-like characters exceed a threshold it picks the code ratio; otherwise it uses the prose ratio. You can override the choice manually."
    },
    {
      question: "How current are the prices?",
      answer:
        "The prices in the table reflect the public list prices from OpenAI, Anthropic, and Google as of mid-2025. Always check the vendor pricing page for the most current numbers — list prices change and there are also batch / cached / prompt-cache discounts not modeled here."
    },
    {
      question: "Does input and output cost the same?",
      answer:
        "No. Output tokens almost always cost more than input tokens (often 3-5x). The tool shows both rates and computes total cost assuming your specified split."
    }
  ]
};
...
export const CONTENT_TYPES = {
  prose: {
    id: "prose",
    name: "Prose",
    description: "Natural-language English text",
    tokensPerChar: 0.26
  },
  code: {
    id: "code",
    name: "Code",
    description: "Source code (high symbol density)",
    tokensPerChar: 0.3
  }
};
...
export function detectContentType(text) {
  ...
  if (
    text.includes("\n") &&
    /\b(function|const|let|var|class|import|export|return|if|else|for|while|def|public|private)\b/.test(
      text
    )
  )
    return "code";
...
function renderStats(
  ...
  statsEl.innerHTML = cells
    .map(
      c =>
        `<div style="background:var(--color-surface);border:1px solid var(--color-border);border-radius:var(--radius-md);padding:var(--space-3);text-align:center;${c.highlight ? "border-color:var(--color-primary);" : ""}"><div style="font-size:var(--text-xs);color:var(--color-text-muted);text-transform:uppercase;letter-spacing:0.5px;">${escapeHtml(c.label)}</div><div style="font-size:${c.highlight ? "var(--text-2xl)" : "var(--text-base)"};font-weight:${c.highlight ? "700" : "600"};margin-top:var(--space-1);">${escapeHtml(String(c.value))}</div></div>`
    )
    .join("");
  ...
  compareBodyEl.innerHTML = compareAllModels(tokens)
    .map(r => {
...
export function render(container) {
  ...
  const els = {
    modelEl: container.querySelector("#ltc-model"),
    ratioEl: container.querySelector("#ltc-ratio"),
    ratioValEl: container.querySelector("#ltc-ratio-val"),
    sampleBtn: container.querySelector("#ltc-sample"),
    clearBtn: container.querySelector("#ltc-clear"),
    textEl: container.querySelector("#ltc-text"),
    highlightEl: container.querySelector("#ltc-highlight"),
    ctBadgeEl: container.querySelector("#ltc-ct-badge"),
    statsEl: container.querySelector("#ltc-stats"),
    modelNameEl: container.querySelector("#ltc-model-name"),
    costTotalEl: container.querySelector("#ltc-cost-total"),
    inputTokensEl: container.querySelector("#ltc-input-tokens"),
    inputCostEl: container.querySelector("#ltc-input-cost"),
    outputTokensEl: container.querySelector("#ltc-output-tokens"),
    outputCostEl: container.querySelector("#ltc-output-cost"),
    compareBodyEl: container.querySelector("#ltc-compare-body")
  };

  els.modelEl.innerHTML = MODELS.map(
    m =>
      `<option value="${escapeHtml(m.id)}" ${m.id === state.modelId ? "selected" : ""}>${escapeHtml(m.provider)} — ${escapeHtml(m.name)}</option>`
  ).join("");
  els.modelEl.addEventListener("change", () => {
    state.modelId = els.modelEl.value;
    renderStats(state, els);
  });
  els.ratioEl.addEventListener("input", () => {
    state.inputRatio = parseInt(els.ratioEl.value, 10) / 100;
    els.ratioValEl.textContent = els.ratioEl.value + "%";
    renderStats(state, els);
  });
  els.textEl.addEventListener("input", () => {
    state.text = els.textEl.value;
    renderStats(state, els);
  });
  els.sampleBtn.addEventListener("click", () => {
    state.text = SAMPLE_TEXT;
    els.textEl.value = state.text;
    renderStats(state, els);
  ...
  container.querySelector("#ltc-copy-json").addEventListener("click", async () => {
    ...
    showToast({
      message: ok ? "Copied stats as JSON" : "Copy failed",
  ...
  container.querySelector("#ltc-copy-md").addEventListener("click", async () => {
    ...
    showToast({
      message: ok ? "Copied stats as Markdown" : "Copy failed",
      type: ok ? "success" : "error"
    });
  });
  renderStats(state, els);
