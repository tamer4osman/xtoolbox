...
function extractImages(doc) {
  ...
  doc.querySelectorAll('img').forEach((img, i) => {
    ...
    images.push({ label: `Image #${i + 1}: ${alt || src.slice(0, 50)}`, content: src, url: src, download: false });
  });
  return images;
}
...
function extractStyles(doc) {
  ...
  doc.querySelectorAll('link[rel="stylesheet"]').forEach((link, i) => {
    ...
    styles.push({ label: `External CSS #${i + 1}: ${href}`, content: `<link rel="stylesheet" href="${escHtml(href)}">`, download: true, filename: `style-${i + 1}.html`, type: 'text/html' });
  });
  doc.querySelectorAll('style').forEach((s, i) => {
    styles.push({ label: `Inline Style #${i + 1}`, content: s.textContent, download: true, filename: `inline-style-${i + 1}.css`, type: 'text/css' });
  });
  return styles;
}
...
function extractFonts(doc, html) {
  ...
  doc.querySelectorAll('link[href*="fonts.googleapis.com"], link[href*="fonts.gstatic.com"]').forEach((link) => {
    ...
    if (!fontUrls.has(href)) {
      fontUrls.add(href);
      fonts.push({ label: `Google Font #${fonts.length + 1}`, content: `<link href="${escHtml(href)}" rel="stylesheet">`, download: true, filename: `font-${fonts.length + 1}.html`, type: 'text/html' });
    }
  });
  ...
  if (inlineFontMatches) {
    ...
    inlineFontMatches.forEach(m => {
      ...
      if (name && !seen.has(name.toLowerCase()) && !skip.includes(name.toLowerCase())) {
        ...
        fonts.push({ label: `Font Family: ${name}`, content: `font-family: '${name}', sans-serif;`, download: false });
      }
    });
  }
  return fonts;
...
function extractScripts(doc) {
  ...
  doc.querySelectorAll('script[src]').forEach((script, i) => {
    ...
    scripts.push({ label: `External Script #${i + 1}: ${src.slice(0, 60)}`, content: `<script src="${escHtml(src)}"><\/script>`, download: true, filename: `script-${i + 1}.html`, type: 'text/html' });
  });
  doc.querySelectorAll('script:not([src])').forEach((script, i) => {
    if (script.textContent.trim()) {
      scripts.push({ label: `Inline Script #${i + 1}`, content: script.textContent, download: true, filename: `inline-script-${i + 1}.js`, type: 'application/javascript' });
    }
  });
  return scripts;
}
