...
export const toolConfig = {
  ...
  steps: [
    ...
    "Adjust the 10 frequency band sliders",
    "Listen to the equalized audio",
    "View the real-time spectrum"
  ],
  faqs: [
    {
      question: "What do the frequency bands control?",
      answer:
        "Each band controls a specific frequency range: bass (low), midrange (middle), and treble (high)."
    },
    {
      question: "Can I reset the equalizer?",
      answer: "Yes, click the Reset button to set all bands back to 0dB (flat response)."
    }
  ]
};
...
export function render(container) {
  ...
  resetBtn.addEventListener("click", () => {
    ...
    EQ_LABELS.forEach((_, i) => {
      container.querySelector(`#gain-${i}`).textContent = "0dB";
    });
    drawEQCurve(filters, eqCurveCanvas, audioContext.sampleRate);
  });
