...
export const toolConfig = {
  ...
  steps: ['Upload an audio file', 'Choose output format', 'Click "Convert"', 'Download converted audio'],
  faqs: [{ question: 'Which format should I choose?', answer: 'MP3 for compatibility, WAV for quality, OGG for web, FLAC for archival.' }]
};
...
export function render(container) {
  ...
  container.querySelector('#format-select').addEventListener('change', (e) => {
    container.querySelector('#bitrate-group').style.display = e.target.value === 'wav' ? 'none' : 'block';
  });
