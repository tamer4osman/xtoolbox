...
export function render(container) {
  ...
  convertBtn.addEventListener('click', async () => {
    ...
    const codecMap = { mp4: ['-c:v', 'libx264', '-c:a', 'aac'], webm: ['-c:v', 'libvpx-vp9', '-c:a', 'libopus'], avi: ['-c:v', 'mpeg4', '-c:a', 'mp3'], mov: ['-c:v', 'libx264', '-c:a', 'aac'] };
    const mimeMap = { mp4: 'video/mp4', webm: 'video/webm', avi: 'video/x-msvideo', mov: 'video/quicktime' };

    try {
      convertBtn.style.display = 'none';
      await runFFmpeg(`output.${format}`, [...codecMap[format], `output.${format}`], mimeMap[format], `converted.${format}`);
