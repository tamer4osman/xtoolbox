...
export const toolConfig = {
  ...
  category: 'video',
  description: 'Burn SRT or VTT subtitle files permanently into your video. Subtitles become part of the video.',
  icon: '💬',
  accept: '.mp4,.webm,.mov,video/mp4,video/webm,video/quicktime',
  maxSizeMB: 500,
  keywords: ['add subtitles', 'burn subtitles', 'srt to video', 'vtt to video', 'hardcode subtitles'],
  steps: ['Upload a video', 'Upload an SRT or VTT file', 'Choose subtitle style', 'Click "Burn Subtitles"', 'Download video'],
  faqs: [
    { question: 'What subtitle formats are supported?', answer: 'SRT and VTT (WebVTT) subtitle files are both supported.' },
    { question: 'Can I remove subtitles afterwards?', answer: 'Once burned in, subtitles are permanent. Keep your original video if you want an unsubbed version.' }
  ]
};
...
export function render(container) {
  ...
  convertBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      await ffmpeg.exec([
        '-i', 'input.mp4',
        '-vf', `subtitles=${subName}:fonts'='size=${fontSize}`,
        '-c:v', 'libx264',
        '-c:a', 'copy',
        '-pix_fmt', 'yuv420p',
        '-movflags', '+faststart',
