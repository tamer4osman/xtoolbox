...
export const toolConfig = {
  ...
  steps: ['Upload a video', 'Upload an audio file', 'Choose replace or merge', 'Download combined video'],
  faqs: [
    { question: 'Can I replace the existing audio?', answer: 'Yes. Choose "Replace" to remove existing audio and use the new track.' }
  ]
};
  ...
  combineBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (mode === 'replace') {
        ...
        args.push('-filter_complex', '[0:a][1:a]amix=inputs=2:duration=first', '-c:v', 'copy', 'output.mp4');
      }
