...
export const toolConfig = {
  ...
  category: "reference",
  description:
    "Compare current times across multiple time zones and convert a specific date/time between zones. Add cities, reorder, and persist your selection.",
  icon: "🌐",
  accept: null,
  maxSizeMB: null,
  keywords: [
  ...
  faqs: [
    {
      ...
      answer:
        'IANA time zones are standard identifiers like "America/New_York" or "Europe/Istanbul". Every modern browser knows how to handle them via the Intl API.'
    },
    {
      question: "Do you handle daylight saving time?",
      answer:
        "Yes. Offsets are computed for the exact moment you are viewing, so DST transitions (spring forward / fall back) are reflected automatically."
    },
    {
      question: "Can I save my favorite cities?",
      answer:
        "Yes. The list of zones is stored in your browser via localStorage and restored on your next visit. Removing all zones will reset to the default set next time."
    },
    {
      question: "Is data sent to a server?",
      answer: "No. Everything happens locally using your browser's built-in Intl and Date APIs."
    }
  ]
};
...
export function render(container) {
  ...
  const state = {
    ...
    baseZone:
      initial.baseZone && initial.zones.includes(initial.baseZone)
        ? initial.baseZone
        : initial.zones[0] || null
  };
  if (state.zones.length === 0) state.zones = getDefaultZones();

  const quickPicks = Object.keys(CITY_LABELS)
  ...
  function renderAll() {
    ...
    baseEl.innerHTML = state.zones
      .map(
        z =>
          `<option value="${z}" ${z === state.baseZone ? "selected" : ""}>${zoneLabel(z)} (${z})</option>`
      )
      .join("");
