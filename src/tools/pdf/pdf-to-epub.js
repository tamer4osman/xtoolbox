...
async function extractPdfText(file) {
  ...
  for (let i = 1; i <= pdf.numPages; i++) {
    ...
    if (cur.length > 0) paragraphs.push(cur.join(' '));
    if (paragraphs.length > 0) chapters.push({ number: i, paragraphs: paragraphs.filter(p => p.trim().length > 0) });
  }
  return chapters;
}
...
function generateEpub(chapters, title, author) {
  ...
  zip.file('OEBPS/content.opf', `<?xml version="1.0" encoding="UTF-8"?><package xmlns="http://www.idpf.org/2007/opf" version="3.0" unique-identifier="uid"><metadata xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:opf="http://www.idpf.org/2007/opf"><dc:identifier id="uid">${uid}</dc:identifier><dc:title>${escapeHtml(title)}</dc:title><dc:language>en</dc:language><dc:creator>${escapeHtml(author)}</dc:creator><meta property="dcterms:modified">${date}</meta></metadata><manifest>${manifestItems}  </manifest><spine toc="ncx">${spineItems}  </spine></package>`);
  ...
  zip.file('OEBPS/cover.xhtml', `<?xml version="1.0" encoding="UTF-8"?><!DOCTYPE html><html xmlns="http://www.w3.org/1999/xhtml"><head><title>Cover</title><link href="style.css" rel="stylesheet" type="text/css"/></head><body><h1>${escapeHtml(title)}</h1><p style="text-align:center;color:#666;">${escapeHtml(author)}</p></body></html>`);
  chapters.forEach((ch, idx) => {
    const id = `chapter${idx + 1}`;
    zip.file(`OEBPS/text/${id}.xhtml`, `<?xml version="1.0" encoding="UTF-8"?><!DOCTYPE html><html xmlns="http://www.w3.org/1999/xhtml"><head><title>Chapter ${ch.number}</title><link href="../style.css" rel="stylesheet" type="text/css"/></head><body><h1>Chapter ${ch.number}</h1>${ch.paragraphs.map(p => `<p>${escapeHtml(p)}</p>`).join('\n      ')}</body></html>`);
  });
