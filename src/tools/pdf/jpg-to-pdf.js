...
export const toolConfig = {
  ...
  steps: ['Upload JPG image(s)', 'Choose page size and margins', 'Click "Convert to PDF"', 'Download the PDF'],
  faqs: [
  ...
  convertBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        const img = images[i];
        const jpgBytes = await fetch(img.src).then(r => r.arrayBuffer());
        const jpgImage = await pdfDoc.embedJpg(new Uint8Array(jpgBytes));
        
        let pageWidth, pageHeight;
        
        if (pageSize === 'fit') {
          pageWidth = jpgImage.width + margin * 2;
          pageHeight = jpgImage.height + margin * 2;
        } else {
          const sizes = {
            'a4': [595, 842],
            'letter': [612, 792],
