...
export const toolConfig = {
  ...
  steps: ['Upload a PDF with form fields', 'Fill in the form fields', 'Click "Download Filled PDF"'],
  faqs: [
    { question: 'What if my PDF has no form fields?', answer: 'You\'ll see a message saying no form fields were found.' },
    { question: 'Are my answers saved?', answer: 'No. Everything is processed in your browser and nothing is stored.' }
  ]
};
...
export function render(container) {
  ...
  const upload = createFileUpload({
    ...
    onFilesSelected: async (files) => {
      ...
      formArea.innerHTML = '<div style="text-align:center;padding:var(--space-4);"><div class="spinner"></div></div>';
      ...
      try {
        ...
        if (fields.length === 0) {
          formArea.innerHTML = '<div style="text-align:center;padding:var(--space-8);color:var(--color-text-muted);">No form fields found in this PDF.</div>';
          return;
        }
          ...
          if (type === 'PDFTextField') {
            ...
            try {
              ...
              options.forEach(opt => {
                ...
                option.textContent = typeof opt === 'string' ? opt : opt.display || opt.value || opt;
    ...
    try {
      ...
      formArea.querySelectorAll('[data-field-name]').forEach(el => {
        ...
        try {
          ...
          else if (type === 'PDFCheckBox') { if (el.checked) field.check(); else field.uncheck(); }
