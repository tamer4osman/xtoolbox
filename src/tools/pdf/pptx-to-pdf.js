...
export const toolConfig = {
  ...
  faqs: [
    ...
    { question: "Are images preserved?", answer: "Yes, images embedded in slides are included." },
    {
      ...
      answer: "Text and basic layout are preserved. Complex animations are not supported."
    }
  ]
};
...
async function extractSlideImages(zip, slidePath, mediaPaths) {
  ...
  for (const blip of doc.getElementsByTagName("a:blip")) {
    ...
    const relsXml = await zip
      .file("ppt/slides/_rels/" + slidePath.split("/").pop() + ".rels")
      ?.async("text");
    if (!relsXml) continue;
...
export function render(container) {
  createSingleFileTool({
    ...
    async onConvert({ file, progress }) {
      ...
      const totalSlides = new DOMParser()
        .parseFromString(presXml, "application/xml")
        .getElementsByTagName("p:sldId").length;
      ...
      const mediaPaths = {};
      zip.forEach((path, f) => {
        if (path.startsWith("ppt/media/")) mediaPaths[path] = f;
      });
