...
export const toolConfig = {
  ...
  faqs: [
    { question: "What formats supported?", answer: "We support EPUB files (.epub) up to 50MB." },
    {
      ...
      answer: "Text content is preserved. Images are not included in the PDF."
    },
    {
      question: "Is formatting preserved?",
      answer:
        "Basic structure (headings, paragraphs) is preserved. Complex formatting may be simplified."
    }
  ]
};
...
async function createPdfFromBlocks(allChapters) {
  ...
  for (const chapter of allChapters) {
    ...
    page._textLines = 0;
    if (chapter.title) {
      page = addText(page, chapter.title, 18, true);
      page._textLines += 2;
    }
    for (const block of chapter.blocks) {
      if (block.type === "heading") {
        page = addText(page, block.content, headingSizes[block.level] || 14, true);
        page._textLines++;
      } else if (block.type === "paragraph" || block.type === "text") {
        page = addText(page, block.content, 11, false);
        page._textLines++;
        ...
        if (f) {
          chapters.push({
            title: href
              ...
              .pop()
              .replace(/\.[^.]+$/, ""),
            blocks: extractTextFromXhtml(await f.async("text"))
          });
        }
      }
      progress(75);
