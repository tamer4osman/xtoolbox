  ...
  faqs: [
    {
      ...
      answer: "This creates a visual signature overlay. Consult local laws for legal requirements."
    }
  ]
};
...
function bindSignEvents(container, signPanel, signatureRef) {
  ...
  const updateTypePreview = () => {
    container.querySelector("#type-preview").textContent =
      container.querySelector("#type-name").value || "Your Signature";
    container.querySelector("#type-preview").style.fontFamily =
      container.querySelector("#sign-font").value;
  };
  ...
  container.querySelector("#sign-upload").addEventListener("change", e => {
    ...
    if (file) {
      ...
      reader.onload = ev => (signatureRef.data = ev.target.result);
...
export function render(container) {
  ...
  container.querySelector("#apply-sign").addEventListener("click", async () => {
    ...
    if (activeTab === "draw") {
      ...
      signatureImage = URL.createObjectURL(
        new Blob([await tempDoc.save()], { type: "application/pdf" })
      );
    } else {
      if (!signatureRef.data) {
    ...
    try {
      ...
      let embed = signatureImage.startsWith("data:image")
        ? await pdfDocNew.embedPng(
            await fetch(signatureImage, { signal: AbortSignal.timeout(15000) }).then(r =>
              r.arrayBuffer()
            )
          )
      ...
      downloadBlob(
        new Blob([await pdfDocNew.save()], { type: "application/pdf" }),
