 ...
 */
export async function createPdfPreview({ file, selectable = false, reorderable = false, onSelectionChange, onReorder }) {
  const container = document.createElement('div');
  container.className = 'pdf-preview';
  container.style.cssText = 'display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:var(--space-4);margin:var(--space-4) 0;';

  // Show loading state
  container.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:var(--space-8);"><div class="spinner"></div><p>Rendering pages...</p></div>';
  ...
  function render() {
    ...
    pageOrder.forEach((originalIndex, displayIndex) => {
      ...
      thumb.style.cssText = 'position:relative;border:2px solid var(--color-border);border-radius:var(--radius-md);overflow:hidden;cursor:pointer;transition:border-color 0.2s;';
      ...
      label.style.cssText = 'position:absolute;bottom:0;left:0;right:0;background:rgba(0,0,0,0.7);color:white;font-size:var(--text-xs);text-align:center;padding:2px;';
      ...
      if (selectable) {
        ...
        checkbox.style.cssText = 'position:absolute;top:4px;right:4px;width:18px;height:18px;accent-color:var(--color-primary);';
        checkbox.addEventListener('change', () => {
          if (checkbox.checked) selectedPages.add(originalIndex);
          else selectedPages.delete(originalIndex);
          thumb.style.borderColor = checkbox.checked ? 'var(--color-primary)' : 'var(--color-border)';
      ...
      if (reorderable) {
        ...
        thumb.addEventListener('dragstart', () => { draggedIndex = displayIndex; thumb.style.opacity = '0.5'; });
        thumb.addEventListener('dragend', () => { thumb.style.opacity = '1'; draggedIndex = null; });
        thumb.addEventListener('dragover', (e) => { e.preventDefault(); thumb.style.borderColor = 'var(--color-secondary)'; });
        thumb.addEventListener('dragleave', () => { thumb.style.borderColor = selectedPages.has(originalIndex) ? 'var(--color-primary)' : 'var(--color-border)'; });
