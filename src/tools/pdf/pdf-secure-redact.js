...
export const toolConfig = {
  ...
  category: "pdf",
  description:
    "Cover sensitive text in PDF pages with black rectangles. Note: this is visual redaction only — the underlying text is hidden but not permanently removed from the file.",
  icon: "🔒",
  accept: ".pdf",
  maxSizeMB: 100,
  ...
  faqs: [
    {
      ...
      answer:
        "No. This is visual redaction only — it draws opaque black rectangles over selected areas. The underlying text is hidden but still technically present in the PDF file. For truly destructive redaction that strips text from the content stream, use dedicated desktop software like Adobe Acrobat Pro."
    },
    {
      question: "Can I undo a redaction rectangle?",
      answer:
        "Yes. Click on any red rectangle to select it, then press the Delete or Backspace key to remove it before applying redactions."
    },
    {
      question: "Are my files uploaded to a server?",
      answer:
        "No. All PDF processing happens entirely in your browser using pdf-lib and PDF.js. Your files never leave your device."
    }
  ]
};
...
export function render(container) {
  ...
  _browser = createPdfPageBrowser({
    ...
    onAction: async api => {
      ...
      if (totalRects === 0) {
        showToast({
          message: "No redaction rectangles drawn. Drag on the page to create one.",
          type: "warning"
        });
        return;
      }
  ...
  function renderPage(index) {
    ...
    overlayPreview.style.cssText =
      "position:absolute;background:rgba(0,0,0,0.5);border:1px dashed #ff0000;display:none;pointer-events:none;z-index:20;";
