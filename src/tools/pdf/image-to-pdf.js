...
export const toolConfig = {
  ...
  faqs: [
    { question: "What image formats are supported?", answer: "JPG, PNG, WebP, GIF, BMP, and SVG." },
    {
      ...
      answer: "Yes, each image becomes a separate page in the PDF."
    },
    {
      question: "Will images be scaled?",
      answer: "Images are scaled to fit the chosen page size while maintaining aspect ratio."
    }
  ]
};
  ...
  createUploadTool({
    ...
    async onConvert({ files, progress }) {
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        const { drawWidth, drawHeight } = fitImageToPage(
          embeddedImage,
          pageWidth,
          pageHeight,
          margin
