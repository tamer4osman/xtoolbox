...
export const toolConfig = {
  ...
  category: "pdf",
  description:
    "Annotate PDFs with highlights, underlines, freehand drawing, shapes, text notes, and stamps. Download the annotated PDF.",
  icon: "✏️",
  accept: ".pdf",
  maxSizeMB: 100,
  ...
  faqs: [
    {
      ...
      answer:
        'Annotations are stored in your browser while you work. Click "Download PDF" to save them permanently into the PDF file.'
    },
    {
      question: "Can I remove an annotation?",
      answer:
        "Yes. Switch to the Select tool (arrow icon) and click an annotation to select it, then press Delete or Backspace to remove it."
    },
    {
      question: "Are my files uploaded to a server?",
      answer:
        "No. All processing happens entirely in your browser using PDF.js and pdf-lib. Your files never leave your device."
    }
  ]
};
...
export function render(container) {
  ...
  _browser = createPdfPageBrowser({
    ...
    onAction: async api => {
      ...
      if (totalAnnotations === 0) {
        showToast({
          message: "No annotations to save. Use the toolbar to annotate the PDF first.",
          type: "warning"
        });
        return;
      }
  ...
  function handleMouseDown(e, canvas) {
    ...
    if (_activeTool === "text") {
      placeTextInput(e, canvas);
      return;
    }
    if (_activeTool === "stamp") {
      placeStamp(e, canvas);
      return;
    }
  ...
  _keydownHandler = e => {
    if (
      ...
      if (
        ...
        _selectedAnnotationIndex < pageAnnotations.length
      ) {
...
function hitTestAnnotation(ann, x, y) {
  ...
  switch (ann.type) {
    ...
    case "rectangle":
      return (
        x >= ann.x - margin &&
        x <= ann.x + ann.w + margin &&
        y >= ann.y - margin &&
        y <= ann.y + ann.h + margin
      );
    case "underline":
      return (
        x >= ann.x - margin && x <= ann.x + ann.w + margin && Math.abs(y - ann.y) <= margin + 4
      );
    case "arrow": {
...
function drawSelectionHighlight(canvas, ann) {
  ...
  switch (ann.type) {
    ...
    case "arrow":
      ctx.strokeRect(
        Math.min(ann.x1, ann.x2) - 8,
        Math.min(ann.y1, ann.y2) - 8,
        Math.abs(ann.x2 - ann.x1) + 16,
        Math.abs(ann.y2 - ann.y1) + 16
      );
      break;
    case "freehand":
...
function embedHighlight(pdfPage, ann, h) {
  pdfPage.drawRectangle({
    x: ann.x * INV_SCALE,
    y: h - (ann.y + ann.h) * INV_SCALE,
    width: ann.w * INV_SCALE,
    height: ann.h * INV_SCALE,
    color: parseColor(ann.color || "#FFEB3B"),
    opacity: 0.35
  });
}
...
function embedUnderline(pdfPage, ann, h) {
  ...
  pdfPage.drawLine({
    ...
    color: parseColor(ann.color || "#FF0000"),
    thickness: (ann.width || 3) * INV_SCALE * 0.5
  });
}
...
async function embedText(pdfDoc, pdfPage, ann, h) {
  const opts = {
    x: ann.x * INV_SCALE,
    y: h - ann.y * INV_SCALE,
    size: (ann.fontSize || 16) * INV_SCALE * 0.5,
    color: parseColor(ann.color || "#FF0000")
  };
  try {
    opts.font = await pdfDoc.embedFont("Helvetica-Bold");
  } catch {
    /* built-in fallback */
  }
...
async function embedStamp(pdfDoc, pdfPage, ann, h) {
  const opts = {
    x: ann.x * INV_SCALE,
    y: h - ann.y * INV_SCALE,
    size: 18 * INV_SCALE * 0.5,
    color: parseColor(ann.color || "#dc2626"),
...
const EMBED_FNS = {
  highlight: embedHighlight,
  underline: embedUnderline,
  rectangle: embedRect,
  arrow: embedArrow,
  freehand: embedFreehand,
  text: embedText,
  stamp: embedStamp
};
