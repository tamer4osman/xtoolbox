  ...
  applyBtn.addEventListener('click', () => {
    process('watermarked.pdf', async (pdfDoc) => {
      ...
      pdfDoc.getPages().forEach(page => {
        ...
        let x, y, rotate;
        if (position === 'center') { x = (width - textWidth) / 2; y = height / 2 - fontSize / 2; rotate = 0; }
        else if (position === 'diagonal') { x = (width - textWidth) / 2; y = height / 2; rotate = 45; }
        else if (position === 'bottom-right') { x = width - textWidth - 50; y = 50; rotate = 0; }
        else { x = (width - textWidth) / 2; y = 50; rotate = 0; }
