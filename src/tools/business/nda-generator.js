...
export function render(container) {
  ...
  el('#generatePdf').addEventListener('click', async () => {
    ...
    try {
      ...
      addLine('This Non-Disclosure Agreement ("Agreement") is entered into as of ' + effDate + ' (the "Effective Date") by and between:');
      y -= 10;

      addLine(state.companyName + ', a [COMPANY TYPE] with offices at [ADDRESS] ("Disclosing Party"); and');
      y -= 10;
      ...
      addLine('"Confidential Information" means any information disclosed by either party that is:');
      ...
      if (state.expirationDate) {
        ...
        addLine('This Agreement remains in effect until terminated by either party with 30 days written notice.');
      }
      y -= 10;
      ...
      const sigY = y;
      page.drawLine({ start: { x: 60, y: sigY }, end: { x: 280, y: sigY }, thickness: 1, color: rgb(0.3, 0.3, 0.3) });
      ...
      y = sigY;
      page.drawLine({ start: { x: 330, y: sigY }, end: { x: 550, y: sigY }, thickness: 1, color: rgb(0.3, 0.3, 0.3) });
