...
export const toolConfig = {
  ...
  category: "business",
  description:
    "Generate freelance service contracts with scope, payment terms, deliverables, and IP ownership.",
  icon: "📝",
...
export function render(container) {
  ...
  el("#generatePdf").addEventListener("click", async () => {
    ...
    try {
      ...
      addPara(
        `This Freelance Service Agreement ("Agreement") is entered into as of ${formatDate(state.startDate)} by and between:`
      );
      y -= 10;
      ...
      const ipText = {
        ...
        license:
          "Freelancer retains ownership. Client receives a non-exclusive, perpetual license to use the work.",
      ...
      addPara(
        "Freelancer is an independent contractor, not an employee. Freelancer is responsible for all taxes and insurance."
      );
      y -= 15;
      ...
      addPara(
        "Either party may terminate with 14 days written notice. Client pays for all work completed up to termination date."
      );
      y -= 15;

      addPara("9. GOVERNING LAW", 40, 10, helveticaBold);
      addPara(
        `This Agreement shall be governed by the laws of the State of ${state.governingLaw}.`
      );
      y -= 30;
      ...
      page.drawText(`Date: ${formatDate(state.startDate)}`, {
        x: 60,
        y: y - 30,
        size: 9,
        font: helvetica,
      ...
      page.drawText("Freelancer Signature", {
        x: 330,
        y: y - 15,
        size: 9,
        font: helvetica,
