...
export function render(container) {
  ...
  function bindExperienceListeners() {
    $$('.exp-company').forEach(el => el.addEventListener('input', (e) => {
      state.experience[Number(e.target.dataset.idx)].company = e.target.value;
      debouncedSave(); renderPreview();
    }));
    $$('.exp-role').forEach(el => el.addEventListener('input', (e) => {
      state.experience[Number(e.target.dataset.idx)].role = e.target.value;
      debouncedSave(); renderPreview();
    }));
    $$('.exp-start').forEach(el => el.addEventListener('input', (e) => {
      state.experience[Number(e.target.dataset.idx)].startDate = e.target.value;
      debouncedSave(); renderPreview();
    }));
    $$('.exp-end').forEach(el => el.addEventListener('input', (e) => {
      state.experience[Number(e.target.dataset.idx)].endDate = e.target.value;
      debouncedSave(); renderPreview();
    }));
    $$('.exp-current').forEach(el => el.addEventListener('change', (e) => {
    ...
    $$('.exp-bullet').forEach(el => el.addEventListener('input', (e) => {
      const ei = Number(e.target.dataset.exp);
      const bi = Number(e.target.dataset.bi);
      state.experience[ei].bullets[bi] = e.target.value;
      debouncedSave(); renderPreview();
    }));
    $$('[data-action="remove-exp"]').forEach(el => el.addEventListener('click', (e) => {
      const idx = Number(e.target.dataset.idx);
      state.experience.splice(idx, 1);
      if (state.experience.length === 0) state.experience.push({ company: '', role: '', startDate: '', endDate: '', current: false, bullets: [''] });
      renderExperience(); debouncedSave(); renderPreview();
    }));
    $$('[data-action="add-bullet"]').forEach(el => el.addEventListener('click', (e) => {
      const idx = Number(e.target.dataset.idx);
      state.experience[idx].bullets.push('');
      renderExperience(); debouncedSave();
    }));
    $$('[data-action="remove-bullet"]').forEach(el => el.addEventListener('click', (e) => {
      const ei = Number(e.target.dataset.exp);
      const bi = Number(e.target.dataset.bi);
  ...
  function bindEducationListeners() {
    $$('.edu-school').forEach(el => el.addEventListener('input', (e) => {
      state.education[Number(e.target.dataset.idx)].school = e.target.value;
      debouncedSave(); renderPreview();
    }));
    $$('.edu-degree').forEach(el => el.addEventListener('input', (e) => {
      state.education[Number(e.target.dataset.idx)].degree = e.target.value;
      debouncedSave(); renderPreview();
    }));
    $$('.edu-field').forEach(el => el.addEventListener('input', (e) => {
      state.education[Number(e.target.dataset.idx)].field = e.target.value;
      debouncedSave(); renderPreview();
    }));
    $$('.edu-gpa').forEach(el => el.addEventListener('input', (e) => {
      state.education[Number(e.target.dataset.idx)].gpa = e.target.value;
      debouncedSave(); renderPreview();
    }));
    $$('.edu-start').forEach(el => el.addEventListener('input', (e) => {
      state.education[Number(e.target.dataset.idx)].startDate = e.target.value;
      debouncedSave(); renderPreview();
    }));
    $$('.edu-end').forEach(el => el.addEventListener('input', (e) => {
      state.education[Number(e.target.dataset.idx)].endDate = e.target.value;
      debouncedSave(); renderPreview();
    }));
    $$('[data-action="remove-edu"]').forEach(el => el.addEventListener('click', (e) => {
      const idx = Number(e.target.dataset.idx);
      state.education.splice(idx, 1);
      if (state.education.length === 0) state.education.push({ school: '', degree: '', field: '', startDate: '', endDate: '', gpa: '' });
      renderEducation(); debouncedSave(); renderPreview();
    }));
  }
  ...
  function bindSkillListeners() {
    $$('.skill-cat').forEach(el => el.addEventListener('input', (e) => {
      state.skills[Number(e.target.dataset.idx)].category = e.target.value;
      debouncedSave(); renderPreview();
    }));
    $$('.skill-items').forEach(el => el.addEventListener('input', (e) => {
      state.skills[Number(e.target.dataset.idx)].items = e.target.value;
      debouncedSave(); renderPreview();
    }));
    $$('[data-action="remove-skill"]').forEach(el => el.addEventListener('click', (e) => {
      const idx = Number(e.target.dataset.idx);
      state.skills.splice(idx, 1);
      if (state.skills.length === 0) state.skills.push({ category: 'Technical', items: '' });
      renderSkills(); debouncedSave(); renderPreview();
    }));
  }
  ...
  function renderPreview() {
    ...
    const hasContent = p.fullName || p.title || p.summary || state.experience.some(e => e.company || e.role) || state.education.some(e => e.school) || state.skills.some(s => s.items);

    if (!hasContent) {
      $('#rb-preview-content').innerHTML = '<p class="rb-p-empty">Start filling in your details to see a live preview.</p>';
      return;
    }
    ...
    const experienceHtml = state.experience.some(e => e.company || e.role)
      ? `<div class="rb-p-section"><div class="rb-p-section-title">Experience</div>${state.experience.filter(e => e.company || e.role).map(exp => `
          <div class="rb-p-entry">
            ...
            ${exp.bullets.filter(b => b.trim()).map(b => `<div class="rb-p-entry-detail">${escapeHtml(b)}</div>`).join('')}
          </div>
        `).join('')}</div>`
      : '';

    const educationHtml = state.education.some(e => e.school)
      ? `<div class="rb-p-section"><div class="rb-p-section-title">Education</div>${state.education.filter(e => e.school).map(edu => `
          <div class="rb-p-entry">
            <div class="rb-p-entry-header">
    ...
    const skillsHtml = allSkills.length
      ? `<div class="rb-p-section"><div class="rb-p-section-title">Skills</div><div class="rb-p-skills-grid">${allSkills.map(s =>
          s.items.split(',').map(sk => sk.trim()).filter(Boolean).map(sk => `<span class="rb-p-skill-tag">${escapeHtml(sk)}</span>`).join('')
        ).join('')}</div></div>`
      : '';
  ...
  function syncAllFromDOM() {
    syncPersonal();
    $$('.exp-company').forEach(el => { state.experience[Number(el.dataset.idx)].company = el.value; });
    $$('.exp-role').forEach(el => { state.experience[Number(el.dataset.idx)].role = el.value; });
    $$('.exp-start').forEach(el => { state.experience[Number(el.dataset.idx)].startDate = el.value; });
    $$('.exp-end').forEach(el => { state.experience[Number(el.dataset.idx)].endDate = el.value; });
    $$('.exp-current').forEach(el => { state.experience[Number(el.dataset.idx)].current = el.checked; });
    $$('.exp-bullet').forEach(el => {
      const ei = Number(el.dataset.exp);
      const bi = Number(el.dataset.bi);
      state.experience[ei].bullets[bi] = el.value;
    });
    $$('.edu-school').forEach(el => { state.education[Number(el.dataset.idx)].school = el.value; });
    $$('.edu-degree').forEach(el => { state.education[Number(el.dataset.idx)].degree = el.value; });
    $$('.edu-field').forEach(el => { state.education[Number(el.dataset.idx)].field = el.value; });
    $$('.edu-gpa').forEach(el => { state.education[Number(el.dataset.idx)].gpa = el.value; });
    $$('.edu-start').forEach(el => { state.education[Number(el.dataset.idx)].startDate = el.value; });
    $$('.edu-end').forEach(el => { state.education[Number(el.dataset.idx)].endDate = el.value; });
    $$('.skill-cat').forEach(el => { state.skills[Number(el.dataset.idx)].category = el.value; });
    $$('.skill-items').forEach(el => { state.skills[Number(el.dataset.idx)].items = el.value; });
  }
  ...
  async function generatePDF() {
    ...
    if (state.template === 'modern') {
      ...
      page.drawRectangle({ x: 0, y: 0, width: sidebarWidth, height, color: rgb(0.145, 0.388, 0.922) });
      let sy = height - 40;
      ...
      sy -= 20;
      if (p.title) { drawSidebar(p.title, 15, sy, font, 9, rgb(0.9, 0.9, 0.9)); sy -= 15; }
      sy -= 10;
      [p.email, p.phone, p.location, p.website].filter(Boolean).forEach(c => {
        const lines = wrapText(c, font, 8, sidebarWidth - 20);
        lines.forEach(line => { drawSidebar(line, 15, sy, font, 8, rgb(0.95, 0.95, 0.95)); sy -= 12; });
      });
      sy -= 15;
      const allSkills = state.skills.filter(s => s.items.trim());
      if (allSkills.length) {
        drawSidebar('SKILLS', 15, sy, fontBold, 9, rgb(1, 1, 1)); sy -= 15;
        allSkills.forEach(s => {
          drawSidebar(s.category, 15, sy, fontBold, 8, rgb(0.9, 0.9, 0.9)); sy -= 12;
          s.items.split(',').map(sk => sk.trim()).filter(Boolean).forEach(sk => {
            drawSidebar(sk, 15, sy, font, 8, rgb(0.85, 0.85, 0.85)); sy -= 11;
          });
          sy -= 3;
        });
      }
      ...
      let ry = height - 40;
      if (p.summary) {
        page.drawText('SUMMARY', { x: rx, y: ry, size: 11, font: fontBold, color: rgb(0.145, 0.388, 0.922) });
        ry -= 3;
        page.drawRectangle({ x: rx, y: ry, width: contentWidth - sidebarWidth - 20, height: 1.5, color: rgb(0.145, 0.388, 0.922) });
        ry -= 15;
      ...
      if (state.experience.some(e => e.company || e.role)) {
        page.drawText('EXPERIENCE', { x: rx, y: ry, size: 11, font: fontBold, color: rgb(0.145, 0.388, 0.922) });
        ry -= 3;
        page.drawRectangle({ x: rx, y: ry, width: contentWidth - sidebarWidth - 20, height: 1.5, color: rgb(0.145, 0.388, 0.922) });
        ry -= 15;
        state.experience.filter(e => e.company || e.role).forEach(exp => {
          page.drawText(exp.role || 'Role', { x: rx, y: ry, size: 10, font: fontBold, color: rgb(0.1, 0.1, 0.1) });
          const dateStr = (exp.startDate ? formatDate(exp.startDate) : '') + (exp.startDate ? ' - ' : '') + (exp.current ? 'Present' : formatDate(exp.endDate));
          page.drawText(dateStr, { x: width - margin - 100, y: ry, size: 8, font, color: rgb(0.5, 0.5, 0.5) });
          ry -= 13;
          page.drawText(exp.company || 'Company', { x: rx, y: ry, size: 9, font, color: rgb(0.3, 0.3, 0.3) });
          ry -= 13;
          exp.bullets.filter(b => b.trim()).forEach(b => {
            wrapText(b, font, 8, contentWidth - sidebarWidth - 35).forEach((line, li) => {
              page.drawText((li === 0 ? '\u2022 ' : '  ') + line, { x: rx + 5, y: ry, size: 8, font, color: rgb(0.25, 0.25, 0.25) });
              ry -= 11;
            });
          });
          ry -= 8;
      ...
      if (state.education.some(e => e.school)) {
        if (ry < 120) ry = 120;
        page.drawText('EDUCATION', { x: rx, y: ry, size: 11, font: fontBold, color: rgb(0.145, 0.388, 0.922) });
        ry -= 3;
        page.drawRectangle({ x: rx, y: ry, width: contentWidth - sidebarWidth - 20, height: 1.5, color: rgb(0.145, 0.388, 0.922) });
        ry -= 15;
        state.education.filter(e => e.school).forEach(edu => {
          page.drawText((edu.degree || 'Degree') + (edu.field ? ' in ' + edu.field : ''), { x: rx, y: ry, size: 10, font: fontBold, color: rgb(0.1, 0.1, 0.1) });
          ry -= 13;
          page.drawText(edu.school || 'School', { x: rx, y: ry, size: 9, font, color: rgb(0.3, 0.3, 0.3) });
          ry -= 13;
        });
      }
    } else {
      const isMinimal = state.template === 'minimal';
      page.drawText(p.fullName || 'Your Name', { x: isMinimal ? width / 2 - font.widthOfTextAtSize(p.fullName || 'Your Name', 20) / 2 : margin, y, size: 20, font: fontBold, color: rgb(0.1, 0.1, 0.1) });
      y -= 20;
      if (p.title) {
        page.drawText(p.title, { x: isMinimal ? width / 2 - font.widthOfTextAtSize(p.title, 11) / 2 : margin, y, size: 11, font, color: rgb(0.4, 0.4, 0.4) });
        y -= 18;
      }
      const contactParts = [p.email, p.phone, p.location, p.website].filter(Boolean);
      if (contactParts.length) {
        const contactStr = contactParts.join('  |  ');
        page.drawText(contactStr, { x: isMinimal ? width / 2 - font.widthOfTextAtSize(contactStr, 8) / 2 : margin, y, size: 8, font, color: rgb(0.5, 0.5, 0.5) });
        y -= 15;
      }
      y -= 5;
      if (!isMinimal) {
        page.drawRectangle({ x: margin, y, width: contentWidth, height: 1, color: rgb(0.8, 0.8, 0.8) });
        y -= 15;
      } else {
        y -= 10;
      }
      ...
      const drawSection = (title) => {
        page.drawText(title.toUpperCase(), { x: margin, y, size: 10, font: fontBold, color: isMinimal ? rgb(0.1, 0.1, 0.1) : rgb(0.145, 0.388, 0.922) });
        y -= 3;
        page.drawRectangle({ x: margin, y, width: contentWidth, height: isMinimal ? 0.5 : 1.5, color: isMinimal ? rgb(0.7, 0.7, 0.7) : rgb(0.145, 0.388, 0.922) });
        y -= 15;
      };
      ...
      if (state.experience.some(e => e.company || e.role)) {
        ...
        state.experience.filter(e => e.company || e.role).forEach(exp => {
          page.drawText(exp.role || 'Role', { x: margin, y, size: 10, font: fontBold, color: rgb(0.1, 0.1, 0.1) });
          const dateStr = (exp.startDate ? formatDate(exp.startDate) : '') + (exp.startDate ? ' - ' : '') + (exp.current ? 'Present' : formatDate(exp.endDate));
          page.drawText(dateStr, { x: width - margin - 100, y, size: 8, font, color: rgb(0.5, 0.5, 0.5) });
          y -= 13;
          page.drawText(exp.company || 'Company', { x: margin, y, size: 9, font, color: rgb(0.3, 0.3, 0.3) });
          y -= 13;
          exp.bullets.filter(b => b.trim()).forEach(b => {
            wrapText(b, font, 8, contentWidth - 15).forEach((line, li) => {
              page.drawText((li === 0 ? '\u2022 ' : '  ') + line, { x: margin + 5, y, size: 8, font, color: rgb(0.25, 0.25, 0.25) });
              y -= 11;
            });
          });
          y -= 8;
      ...
      if (state.education.some(e => e.school)) {
        if (y < 120) { page.drawText('', { x: margin, y }); y = height - margin; }
        drawSection('Education');
        state.education.filter(e => e.school).forEach(edu => {
          page.drawText((edu.degree || 'Degree') + (edu.field ? ' in ' + edu.field : ''), { x: margin, y, size: 10, font: fontBold, color: rgb(0.1, 0.1, 0.1) });
          const dateStr = (edu.startDate ? formatDate(edu.startDate) : '') + (edu.startDate ? ' - ' : '') + formatDate(edu.endDate);
          page.drawText(dateStr, { x: width - margin - 100, y, size: 8, font, color: rgb(0.5, 0.5, 0.5) });
          y -= 13;
    ...
    const filename = (p.fullName || 'resume').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') + '-resume.pdf';
  ...
  $$('#rb-name, #rb-title, #rb-email, #rb-phone, #rb-location, #rb-website, #rb-summary').forEach(el => {
    el.addEventListener('input', () => { syncPersonal(); debouncedSave(); renderPreview(); });
  });
  ...
  $('#rb-add-experience').addEventListener('click', () => {
    state.experience.push({ company: '', role: '', startDate: '', endDate: '', current: false, bullets: [''] });
    renderExperience(); debouncedSave();
  });

  $('#rb-add-education').addEventListener('click', () => {
    state.education.push({ school: '', degree: '', field: '', startDate: '', endDate: '', gpa: '' });
    renderEducation(); debouncedSave();
  });

  $('#rb-add-skill').addEventListener('click', () => {
    state.skills.push({ category: '', items: '' });
    renderSkills(); debouncedSave();
  });
  ...
  $('#rb-export-btn').addEventListener('click', () => {
    ...
    const name = (state.personal.fullName || 'resume').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
