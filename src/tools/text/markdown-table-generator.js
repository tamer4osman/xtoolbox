...
export const toolConfig = {
  ...
  category: "text",
  description:
    "Visually create and edit Markdown tables with a spreadsheet-like interface. Import from CSV/TSV/pipe-delimited or Markdown; export to GitHub-flavored Markdown.",
  icon: "📊",
  ...
  faqs: [
    {
      question: "How do I change column alignment?",
      answer:
        "Each column header has an alignment dropdown (left, center, right). The separator row in the Markdown output updates to match — `:---`, `:---:`, or `---:`."
    },
    {
      question: "What happens if my cell contains a pipe character?",
      answer:
        "Pipes inside cells are escaped as `\\|` so the table stays well-formed. Newlines inside cells become `<br>` tags (GFM convention)."
    },
    {
      question: "Can I import an existing Markdown table?",
      answer:
        "Yes. Paste any standard Markdown table into the Import box and click Import. The tool detects the header row, separator, alignment hints, and body rows."
    },
    {
      question: "Which delimiters does the importer auto-detect?",
      answer:
        "Markdown (header + separator), then tabs, pipes, or commas — whichever appears most in the first non-empty line. You can also pick the delimiter manually."
    }
  ]
};
...
export function generateMarkdown(headers, rows, alignments) {
  ...
  const bodyLines = (rows || []).map(
    r => "| " + r.map((c, i) => pad(escapeCell(c == null ? "" : c), widths[i])).join(" | ") + " |"
  );
...
function bindMarkdownTableEvents(ctx) {
  ...
  gridEl.addEventListener("input", e => {
    if (e.target.classList.contains("mtg-h")) {
      state.headers[parseInt(e.target.dataset.col, 10)] = e.target.value;
      renderAll();
    } else if (e.target.classList.contains("mtg-c")) {
      state.rows[parseInt(e.target.dataset.row, 10)][parseInt(e.target.dataset.col, 10)] =
        e.target.value;
      renderAll();
    }
  });
  ...
  gridEl.addEventListener("change", e => {
    if (e.target.classList.contains("mtg-a")) {
      state.alignments[parseInt(e.target.dataset.col, 10)] = e.target.value;
      renderAll();
    }
  });
  ...
  gridEl.addEventListener("click", e => {
    ...
    if (rmRow) {
      state.rows.splice(parseInt(rmRow.dataset.row, 10), 1);
      renderAll();
    }
  });
  ...
  container.querySelector("#mtg-copy").addEventListener("click", async () => {
    ...
    showToast({
      message: ok ? "Copied Markdown to clipboard" : "Copy failed",
...
export function render(container) {
  ...
  const els = {
    gridEl: q("mtg-grid"),
    outputEl: q("mtg-output"),
    statsEl: q("mtg-stats"),
    importTextEl: q("mtg-import-text"),
    delimEl: q("mtg-delim")
  };

  function renderGrid() {
    ensureShape(state);
    els.gridEl.innerHTML = buildGridHTML(state.headers, state.rows, state.alignments);
  }
