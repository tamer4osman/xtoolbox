...
export const toolConfig = {
  ...
  faqs: [
    {
      question: "How are rows converted?",
      answer:
        "Each row becomes a <row> element with columns as child elements. Column names are used as tag names."
    },
    {
      question: "What about special characters?",
      answer: "All special characters are properly XML-escaped."
    }
  ]
};
...
export function render(container) {
  ...
  const upload = createFileUpload({
    ...
    onFilesSelected: async files => {
      ...
      sheetSelect.innerHTML = sheets
        .map((s, i) => `<option value="${i}">${s.sheet}</option>`)
        .join("");
  ...
  downloadBtn.addEventListener("click", () => {
    if (xmlOutput)
      downloadBlob(
        new Blob([xmlOutput], { type: "application/xml" }),
        currentFile.name.replace(/\.\w+$/, ".xml")
      );
  });
}
