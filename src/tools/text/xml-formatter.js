...
export const toolConfig = {
  ...
  accept: '.xml',
  maxSizeMB: 5,
  keywords: ['xml', 'formatter', 'validator', 'beautifier', 'pretty print', 'xml validator', 'xml formatter'],
  steps: ['Paste or upload XML content', 'Click "Format" to pretty-print', 'View validation results and syntax highlighting', 'Copy or download formatted XML'],
  faqs: [
    { question: 'What does XML formatting do?', answer: 'XML formatting (pretty-printing) adds proper indentation and line breaks to make XML documents human-readable while preserving structure.' },
    { question: 'How does validation work?', answer: 'The tool uses the browser\'s built-in DOMParser to check if the XML is well-formed. It reports line numbers for any parsing errors.' },
    { question: 'Is my XML data sent to a server?', answer: 'No, all processing happens locally in your browser. Your XML data never leaves your device.' }
  ]
};
...
export function render(container) {
  ...
  function showStatus(message, type) {
    ...
    status.style.background = type === 'error' ? 'var(--color-error-bg, #fef2f2)' : 
                             type === 'success' ? 'var(--color-success-bg, #f0fdf4)' : 
                             'var(--color-info-bg, #f0f9ff)';
    status.style.color = type === 'error' ? 'var(--color-error, #dc2626)' : 
                        type === 'success' ? 'var(--color-success, #16a34a)' : 
                        'var(--color-info, #2563eb)';
    status.style.border = `1px solid ${type === 'error' ? 'var(--color-error-border, #fecaca)' : 
                         type === 'success' ? 'var(--color-success-border, #bbf7d0)' : 
  ...
  container.querySelector('.tool-layout').insertBefore(uploadBtn, container.querySelector('.form-group'));
}
