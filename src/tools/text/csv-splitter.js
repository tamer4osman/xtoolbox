...
export const toolConfig = {
  ...
  category: "text",
  description:
    "Split large CSV files into smaller files by row count. All files are packaged in a ZIP.",
  icon: "✂️",
  accept: ".csv",
  maxSizeMB: 50,
  ...
  faqs: [
    {
      question: "What happens to the headers?",
      answer: "Headers are included in every split file so each file is self-contained."
    },
    {
      question: "What if rows don't divide evenly?",
      answer: "The last file will have fewer rows — that's normal."
    }
  ]
};
...
export function render(container) {
  ...
  function updatePreview() {
    ...
    const rpf =
      rowsSelect.value === "custom"
        ? parseInt(customRowsInput.value, 10) || 500
  ...
  convertBtn.addEventListener("click", async () => {
    ...
    const rpf =
      rowsSelect.value === "custom"
        ? parseInt(customRowsInput.value, 10)
        : parseInt(rowsSelect.value, 10);
    if (!rpf || rpf < 1) {
