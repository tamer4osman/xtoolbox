...
export const toolConfig = {
  ...
  accept: null,
  maxSizeMB: null,
  keywords: ['citation generator', 'apa citation', 'mla citation', 'chicago citation', 'bibliography'],
  steps: ['Enter source details', 'Choose citation style', 'Copy formatted citation'],
  faqs: [
    { question: 'What citation styles are supported?', answer: 'APA 7th Edition, MLA 9th Edition, and Chicago 17th Edition (Notes-Bibliography).' },
    { question: 'What source types are supported?', answer: 'Book, Journal Article, Website, and Newspaper Article.' }
  ]
};

function formatAPA(data) {
  const { author, year, title, journal, volume, issue, pages, publisher, url, doi, sourceType } = data;
  const a = author.trim();
  const y = year.trim();
  const t = title.trim();
...
export function render(container) {
  ...
  const FIELDS = {
    ...
    website: [
      ...
      { id: 'year', label: 'Publication / Access Date', placeholder: '2024 or Month Day, Year', required: true },
  ...
  function generate() {
    ...
    if (missing) {
      outputEl.innerHTML = '<span style="color:var(--color-text-muted);">Fill in required fields to generate citation...</span>';
      return;
    }
