...
export const toolConfig = {
  ...
  category: "css",
  description:
    "Upload web fonts and subset them to only the glyphs you need. Supports language presets and custom Unicode ranges.",
  icon: "🔤",
...
async function initOpentype(container) {
  ...
  script.onerror = () => {
    container.querySelector("#fileUploadContainer").innerHTML =
      '<p class="error">Failed to load font library. Please refresh and try again.</p>';
  };
...
async function loadFont(file, container) {
  ...
  try {
    ...
    fontFamily.textContent =
      fontData.names.fontFamily?.en || fontData.names.fullName?.en || state.fontName;
