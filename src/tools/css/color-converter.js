...
function formatHsl(v) {
  return `hsl(${v.h}, ${v.s}%, ${v.l}%)`;
}
function formatHsv(v) {
  return `hsv(${v.h}, ${v.s}%, ${v.v}%)`;
}
function formatCmyk(v) {
  ...
  if (hexMatch) {
    ...
    if (rgb)
      return {
        hex: "#" + h.toLowerCase(),
        rgb: formatRgb(rgb),
        hsl: formatHsl(rgbToHsl(rgb.r, rgb.g, rgb.b)),
        hsv: formatHsv(rgbToHsv(rgb.r, rgb.g, rgb.b)),
        cmyk: formatCmyk(rgbToCmyk(rgb.r, rgb.g, rgb.b))
      };
  }
  ...
  if (rgbMatch) {
    ...
    if (r <= 255 && g <= 255 && b <= 255) {
      ...
      return {
        hex: rgbToHex(r, g, b),
        rgb: formatRgb(rgb),
        hsl: formatHsl(rgbToHsl(r, g, b)),
        hsv: formatHsv(rgbToHsv(r, g, b)),
        cmyk: formatCmyk(rgbToCmyk(r, g, b))
      };
    }
  }
...
export const toolConfig = {
  ...
  steps: [
    ...
    "See conversions to all formats instantly",
    "Copy any format with one click"
  ],
  faqs: [
    {
      question: "What color formats are supported?",
      answer: "HEX (#ff0000), rgb(r,g,b), hsl(h,s%,l%), HSV, and CMYK."
    },
    {
      question: "How accurate are the conversions?",
      answer:
        "Conversions use standard mathematical formulas. Some precision loss may occur between formats."
    }
  ]
};
