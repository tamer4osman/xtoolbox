...
function renderMetaTags(meta) {
  ...
  return tags.map(t => `<div class="meta-tag"><span class="meta-key">${t.label}</span><span class="meta-val">${truncate(t.value, 80) || '<em>missing</em>'}</span></div>`).join('');
}
...
export function render(container) {
  ...
  fetchBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      const warns = [];
      if (!meta.title) warns.push({ level: 'error', msg: 'Missing og:title — platforms will use page title or show blank' });
      else if (meta.title.length > 60) warns.push({ level: 'warning', msg: `og:title is ${meta.title.length} chars — may be truncated on Twitter (60 chars optimal)` });

      if (!meta.description) warns.push({ level: 'error', msg: 'Missing og:description — platforms will auto-extract or show blank' });
      else if (meta.description.length > 160) warns.push({ level: 'warning', msg: `og:description is ${meta.description.length} chars — may be truncated (160 optimal)` });

      if (!meta.image) warns.push({ level: 'error', msg: 'Missing og:image — link will share without a preview image' });

      if (meta.image && meta.image.startsWith('/')) warns.push({ level: 'error', msg: 'og:image uses a relative path — will break when shared (must be absolute URL)' });

      warnings.innerHTML = warns.length > 0
